...
for ($j = 0; $j < sizeof($body->uplinks); $j++) {
    ...
    if (!$row[0]) {
        if($cid.$sid == 0205)
        {
            echo 'inside 0205 ' . 'INSERT INTO raw_data_b64 (device_id, b64_data, timestamp, message_id) VALUES ("' . $nodeId . '" , "' . $b64 . '" , "' . $timestamp . '", "' . $messageId . '") ON DUPLICATE KEY UPDATE message_id = VALUES(message_id)';
        }
         $stmt = $db->unprepared_query('INSERT INTO raw_data_b64 (device_id, b64_data, timestamp, message_id) VALUES ("' . $nodeId . '" , "' . $b64 . '" , "' . $timestamp . '", "' . $messageId . '") ON DUPLICATE KEY UPDATE message_id = VALUES(message_id)');
        print_r($stmt);
    }
