       ...
       // Log::debug('dmtImportHawk entering foreach');
        foreach($data['Records'] as $row)
        {
            ...
            foreach($row['Fields'] as $field)
            {
                ...
                if($type == 6){ // analog
                    ...
                    $bp   = !empty($field["AnalogueData"]['1']) ? ( ( (float) $field["AnalogueData"]['1'] ) / 1200  ) : 0; // Batt.Percentage.
                    // backfill
                    if(!empty($m_items) && is_array($m_items) && count($m_items) > 0){
                        foreach($m_items as &$m){
                            if(is_object($m)){
            ...
            if(!empty($n_values)){
                foreach($n_values as $key => $val){
                    ...
                    $item = new nutrients_data();
                    
                    $item->node_address = $node_address; $item->probe_serial = $probe_serial;
                    $item->vendor_model_fw = $vendor_model_fw; $item->version = $node_ver;
                    $item->identifier = $key; $item->value = $val;
                    ...
                    $item->message_id = "dmt_{$node_address}_{$key}_{$date_sampled}";
        ...
        if($n_items){
            foreach($n_items as &$n_item){
                if(is_object($n_item)){
                    if(!nutrients_data::where('message_id', $n_item->message_id)->exists()){
                        $n_item->save();
                    }
                }
            }
