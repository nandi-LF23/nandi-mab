        ...
        <template #default="{ hide }">
              ...
              <b-row align-v="center">
                <b-col md>
                  ...
                  <base-input v-if="entity_select.value == 'Child'" name="entity" rules="required">
                    <el-select v-model="model.parent" value-key="id" filterable placeholder="Select">
                      <el-option v-for="item in entities" :key="item.company_name"
                        :label="item.company_name" :value="item.id"></el-option>
                    </el-select>
                  </base-input>
          ...
          <validation-observer ref='addstep_basic' slim>
            <form role="addstep_basic" v-show="formStep == 'basic'" @submit.prevent="() => false">
              <b-row>
                <b-col md>
                  <h3>Basic Info</h3>
                  <base-input label="Entity Name" name="entity name" rules="required" placeholder="Entity Name"
                    vid="company_name" v-model="model.company_name"></base-input>
                  <base-input label="Entity Logo" name="entity logo">
                    <ImageCropUpload :stencilProps="{ scalable: true, movable: true/*, aspectRatio: 2/1*/ }"
                      :resize="{ height: 32, width: 'auto' }" @clear="clearLogo" @done="setLogo" />
                  </base-input>
                  <base-input label="Contact Name" name="contact name" placeholder="Contact Name" vid="contact_name"
                    v-model="model.contact_name" rules="required">
                  </base-input>
                  <base-input label="Contact Email" name="contact email" placeholder="Contact Email" vid="contact_email"
                    v-model="model.contact_email"></base-input>
                  <base-input label="Contact Phone" name="contact phone" placeholder="Contact Phone" vid="contact_phone"
                    v-model="model.contact_phone"></base-input>
                </b-col>
              </b-row>
            </form>
          </validation-observer>
            ...
            <form role="addstep_paddr" v-show="formStep == 'paddr'" @submit.prevent="() => false">
              <b-row>
                <b-col md>
                  ...
                  <base-input name="physical city" placeholder="Physical Town/City" vid="physical_address.city"
                    v-model="model.physical_address.city"></base-input>
                  <base-input name="physical postal code" placeholder="Physical Postal Code"
                    vid="physical_address.postalcode" v-model="model.physical_address.postalcode"></base-input>

                  <base-input name="physical country" placeholder="Physical Country" vid="physical_address.country">
                    <el-select v-model="model.physical_address.country" filterable placeholder="Country">
              ...
              <b-row>
                <b-col md>
                  ...
                  <base-input name="billing address line 2" placeholder="Billing Address Line 2"
                    ...
                    :disabled="model.p_as_b == 'yes'"></base-input>
                  <base-input name="billing city" placeholder="Billing Town/City" vid="billing_address.city"
              ...
              <b-row>
                <b-col md>
                  ...
                  <base-input name="modules" vid="modules" rules="required">
                    <b-form-checkbox size="lg" v-model="model.modules" unchecked-value="" value="Soil Moisture">Soil
                      Moisture</b-form-checkbox>
                    <b-form-checkbox v-if="isAdmin()" size="lg" v-model="model.modules" unchecked-value=""
                      value="Nutrients">Nutrients</b-form-checkbox>
                    <b-form-checkbox size="lg" v-model="model.modules" unchecked-value="" value="Well Controls">Well
          ...
          <b-row v-show="formStep == 'confirm'">
            <b-col>
              ...
              <b-row>
                <b-col>
                  <b-table-simple v-model="tableData.company_name" value-key="id" responsive small>
                    <b-tr><b-td><strong>Type of Entity</strong></b-td></b-tr>
                    <b-tr><b-td>{{ entity_select.value }}</b-td>
                      <base-input class="mt-2" v-if="entity_select.value == 'Child'" name="entity" rules="required">
                        <el-select v-model="tableData.company_name" value-key="id" filterable placeholder="Select">
                          <el-option v-for="item in tableData" v-if="item.is_distributor == 0" :key="item.company_name"
                            :label="item.company_name" :value="item.id"></el-option>
                        </el-select>
                      </base-input>
                    </b-tr>
              ...
              <b-row>
                <b-col>
                  <b-table-simple responsive small>
                    ...
                    <b-tr><b-td>{{ model.contact_name }}</b-td><b-td>{{ model.contact_email }}</b-td><b-td>{{
              ...
              <b-row>
                <b-col>
                  <b-table-simple responsive small>
                    ...
                    <b-tr><b-td>Line 1</b-td><b-td>{{ model.physical_address.line_1 }}</b-td><b-td>{{ model.p_as_b ==
                      'yes' ? model.physical_address.line_1 : model.billing_address.line_1 }}</b-td></b-tr>
                    <b-tr><b-td>Line 2</b-td><b-td>{{ model.physical_address.line_2 }}</b-td><b-td>{{ model.p_as_b ==
                      'yes' ? model.physical_address.line_2 : model.billing_address.line_2 }}</b-td></b-tr>
                    <b-tr><b-td>City/Town</b-td><b-td>{{ model.physical_address.city }}</b-td><b-td>{{ model.p_as_b ==
                      'yes' ? model.physical_address.city : model.billing_address.city }}</b-td></b-tr>
                    <b-tr><b-td>Postal Code</b-td><b-td>{{ model.physical_address.postalcode }}</b-td><b-td>{{
                      model.p_as_b == 'yes' ? model.physical_address.postalcode : model.billing_address.postalcode
                    }}</b-td></b-tr>
                    <b-tr><b-td>Country</b-td><b-td>{{ model.physical_address.country }}</b-td><b-td>{{ model.p_as_b ==
                      'yes' ? model.physical_address.country : model.billing_address.country }}</b-td></b-tr>
                  </b-table-simple>
                </b-col>
              </b-row>
          ...
          <validation-observer ref='remcc' v-slots="{ validate, reset }" slim>
            <form class='cc_move_form' role="form" @submit.prevent="() => false">
              ...
              <b-row>
                <b-col md>
                  <!-- Move Entity Option -->
                  <b-form-checkbox size="lg" v-model="move_model.move_company" name="move_company" :value="true"
                  ...
                  <b-col md>
                    ...
                    <base-input label="Destination Entity" rules="required" vid="destination_entity"
                      ...
                      <el-select @change="getNewCompanyMoveMeta(move_model.new_company_id)"
                        v-model="move_model.new_company_id" filterable value-key="id" placeholder="Choose Entity">
                ...
                <b-row v-if="move_model.new_company_id">
                  <b-col md>
                    <base-input label="Choose Objects" rules="required" vid="move_types" name="chosen objects">
                      <b-form-group>
                        <b-form-checkbox-group @change="reset" size="lg" v-model="move_model.move_types"
                  ...
                  <b-col md>
                    ...
                    <b-table-simple responsive small
                      ...
                      <b-tr>
                        ...
                        <b-th class='text-right'><strong>{{
                          move_model.target_companies['"' + move_model.new_company_id.toString() + '"'].company_name
                        }}</strong></b-th>
                      </b-tr>
                      <template v-for="(sensormap, index) in move_model.sensor_mappings">
                        <b-tr :key="sensormap.old_sensor_id">
                          <template v-if="move_model.counts[sensormap.old_sensor_type]">
                            ...
                            <b-td class='valign'>
                              <base-input :vid="'new_sensors_' + index" rules="required" name="new sensor choice">
                                <el-select v-model="sensormap.new_sensor_id" value-key="id" size="small" filterable
                                  rules="required" placeholder="New Sensor">
                                  <el-option v-for="sensor in move_model.new_sensors"
                                    v-if="sensormap.old_sensor_type == sensor.device_type" :key="sensor.id"
                                    :label="sensor.device_make" :value="sensor.id">
                                  </el-option>
                                </el-select>
                              </base-input>
                            </b-td>
                    ...
                    <p v-else>
                      ...
                      <template v-else-if="move_model.new_sensors.length == 0">
                        <strong>{{ move_model.target_companies['"' + move_model.new_company_id.toString() +
                          '"'].company_name
                    ...
                    <b-table-simple responsive small
                      ...
                      <template v-for="(rolemap, index) in move_model.role_mappings">
                        <b-tr :key="rolemap.old_role_id">
                          ...
                          <b-td class='valign'>
                            <base-input :vid="'new_roles_' + index" rules="required" name="new role choice">
                              <el-select v-model="rolemap.new_role_id" value-key="id" size="mini" filterable
                                rules="required" placeholder="New Role">
                                <el-option v-for="role in move_model.new_roles" :key="role.id" :label="role.role_name"
                                  :value="role.id">
                                </el-option>
                              </el-select>
                            </base-input>
      ...
      <card class="no-border-card" body-classes="px-0 pb-1" footer-classes="pb-2">
              ...
              <template #cell(actions)="data" v-if="canAction">
                <div class="d-flex justify-content-center">
                  <b-button :disabled="!userCan('Edit', 'Entities', data.item.id, 'C')"
                    @click="handleEdit(data.index, data.item)" class="btn" size="sm" variant="outline-primary" icon>
                    Configure
                  </b-button>
        ...
        <div slot="footer" class='align-right'>
          <b-row>
            ...
            <b-col md>
              <b-pagination @input="loadCompanies" v-model="currentPage" :total-rows="totalRows" :per-page="perPage"
...
export default {
  ...
  methods: {
      ...
      loadCompanies() {
          ...
          this.loading = true;
          this.$axios.post("/api/companiesAll", {}).then((resp) => {
              this.entities = resp.data.rows;
    ...
    createCompany() {
      this.model.child_parent = this.entity_select.value;
      if(this.model.child_parent == 'child')
      //this.model.parent = this.tableData.company_name;
