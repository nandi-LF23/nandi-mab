...
class GraphController extends Controller
{
    ...
    public function sm_graph(Request $request)
    {
        ...
        $graph_type = !empty($graph_type) && $hwconfig->node_type == 'Nutrients' && !in_array($graph_type, ['nutrient', 'nutrient_ppm', 'nutrient_type_avg', 'nutrient_sm_sep_levels', 'nutrient_sm_avg', 'nutrient_temp_avg']) ? 'nutrient_type_avg' : $graph_type;
        ...
        if (in_array($graph_type, ['sum', 'ave', 'temp', 'sm', 'stack', 'tech'])) {
            ...
            $node_data = $query->get();
        } else if (in_array($graph_type, ['nutrient', 'nutrient_ppm', 'nutrient_type_avg', 'nutrient_sm_sep_levels', 'nutrient_sm_avg', 'nutrient_temp_avg'])) {
            ...
            case 'nutrient_ppm':
                ...
                if ($ntpl && $tpl_values) {
                    ...
                    foreach ($nutr_data as $item) {
                        ...
                        $dataset = nutrient_template_data::where('nutriprobe', $request->node_address)->limit(1)->first();

                        $polynomials = nutrient_templates::where('id', $dataset->M3_1)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M3_1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M3_1 * $template->poly1) + $template->poly2), 1, '.', ''),
                            (float)$item->M3_1
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M3_2)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M3_2, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M3_2 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M3_3)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M3_3, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M3_3 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M3_4)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M3_4, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M3_4 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);

                        $polynomials = nutrient_templates::where('id', $dataset->M4_1)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M4_1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M4_1 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M4_2)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M4_2, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M4_2 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M4_3)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M4_3, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M4_3 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M4_4)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M4_4, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M4_4 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);

                        $polynomials = nutrient_templates::where('id', $dataset->M5_1)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M5_1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M5_1 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M5_2)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M5_2, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M5_2 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M5_3)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M5_3, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M5_3 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M5_4)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M5_4, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M5_4 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M6_1)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M6_1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M6_1 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M6_2)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M6_2, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M6_2 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M6_3)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M6_3, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M6_3 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                        $polynomials = nutrient_templates::where('id', $dataset->M6_4)->first(); //$conn->query("SELECT * FROM nutrient_templates WHERE id=".$dataset->{'M'.$i.'_'.$j});
                        $template = json_decode($polynomials->template);
                        array_push($M6_4, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M6_4 * $template->poly1) + $template->poly2), 1, '.', ''),
                        ]);
                    ...
                    if ($user->unit_of_measure  == 2) {
                        ...
                        $N03_label1 = "N03(100mm)";
                        $N03_label2 = "N03(300mm)";
                        $NH4_label1 = "NH4(200mm)";
                        $NH4_label2 = "NH4(400mm)";
                    }

                    $series[] = ['name' => 'M3.1 ' . $N03_label1, 'type' => 'spline', 'data' => $M3_1];
                    $series[] = ['name' => 'M3.2 ' . $NH4_label1, 'type' => 'spline', 'data' => $M3_2];
                    $series[] = ['name' => 'M3.3 ' . $N03_label2, 'type' => 'spline', 'data' => $M3_3];
                    $series[] = ['name' => 'M3.4 ' . $NH4_label2, 'type' => 'spline', 'data' => $M3_4];

                    $series[] = ['name' => 'M4.1 ' . $N03_label1, 'type' => 'spline', 'data' => $M4_1];
                    $series[] = ['name' => 'M4.2 ' . $NH4_label1, 'type' => 'spline', 'data' => $M4_2];
                    $series[] = ['name' => 'M4.3 ' . $N03_label2, 'type' => 'spline', 'data' => $M4_3];
                    $series[] = ['name' => 'M4.4 ' . $NH4_label2, 'type' => 'spline', 'data' => $M4_4];

                    $series[] = ['name' => 'M5.1 ' . $N03_label1, 'type' => 'spline', 'data' => $M5_1];
                    $series[] = ['name' => 'M5.2 ' . $NH4_label1, 'type' => 'spline', 'data' => $M5_2];
                    $series[] = ['name' => 'M5.3 ' . $N03_label2, 'type' => 'spline', 'data' => $M5_3];
                    $series[] = ['name' => 'M5.4 ' . $NH4_label2, 'type' => 'spline', 'data' => $M5_4];

                    $series[] = ['name' => 'M6.1 ' . $N03_label1, 'type' => 'spline', 'data' => $M6_1];
                    $series[] = ['name' => 'M6.2 ' . $NH4_label1, 'type' => 'spline', 'data' => $M6_2];
                    $series[] = ['name' => 'M6.3 ' . $N03_label2, 'type' => 'spline', 'data' => $M6_3];
                    $series[] = ['name' => 'M6.4 ' . $NH4_label2, 'type' => 'spline', 'data' => $M6_4];
            ...
            case 'nutrient_ppm_avg':
                ...
                if (is_array($tpl_values)) {
                    ...
                    foreach ($nutr_data as $item) {
                        ...
                        $polynomials = nutrient_templates::where('id', $dataset->M3_1)->first();
                        $template = json_decode($polynomials->template);
                        ...
                        if (isset($item->M3_1)) {
                            ...
                            $polynomials = nutrient_templates::where('id', $dataset->M3_1)->first();
                            $template = json_decode($polynomials->template);
                            $M3_val = ($item->M3_1 * $template->poly1) + $template->poly2;
                        }
                        if (isset($item->M3_2)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M3_2)->first();
                            $template = json_decode($polynomials->template);
                            $M3_val += ($item->M3_2 * $template->poly1) + $template->poly2;
                        }
                        if (isset($item->M3_3)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M3_3)->first();
                            $template = json_decode($polynomials->template);
                            $M3_val += (($item->M3_3 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M3_4)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M3_4)->first();
                            $template = json_decode($polynomials->template);
                            $M3_val += (($item->M3_4 * $template->poly1) + $template->poly2);
                        }
                        ...
                        if (isset($item->M4_1)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M4_1)->first();
                            $template = json_decode($polynomials->template);
                            $M4_val = (($item->M4_1 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M4_2)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M4_2)->first();
                            $template = json_decode($polynomials->template);
                            $M4_val += (($item->M4_2 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M4_3)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M4_3)->first();
                            $template = json_decode($polynomials->template);
                            $M4_val += (($item->M4_3 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M4_4)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M4_4)->first();
                            $template = json_decode($polynomials->template);
                            $M4_val += (($item->M4_4 * $template->poly1) + $template->poly2);
                        }
                        ...
                        if (isset($item->M5_1)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M5_1)->first();
                            $template = json_decode($polynomials->template);
                            $M5_val = (($item->M5_1 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M5_2)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M5_2)->first();
                            $template = json_decode($polynomials->template);
                            $M5_val += (($item->M5_2 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M5_3)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M5_3)->first();
                            $template = json_decode($polynomials->template);
                            $M5_val += (($item->M5_3 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M5_4)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M5_4)->first();
                            $template = json_decode($polynomials->template);
                            $M5_val += (($item->M5_4 * $template->poly1) + $template->poly2);
                        }
                        ...
                        if (isset($item->M6_1)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M6_1)->first();
                            $template = json_decode($polynomials->template);
                            $M6_val = ($item->M6_1 * $poly1) + $poly2;
                        }
                        if (isset($item->M6_2)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M6_2)->first();
                            $template = json_decode($polynomials->template);
                            $M6_val += (($item->M6_2 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M6_3)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M6_3)->first();
                            $template = json_decode($polynomials->template);
                            $M6_val += (($item->M6_3 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M6_4)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M6_4)->first();
                            $template = json_decode($polynomials->template);
                            $M6_val += (($item->M6_4 * $template->poly1) + $template->poly2);
                        }
                        ...
                        array_push($M3_2, [
                        ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M3_2 * $template->poly1) + $template->poly2), 2, '.', ''),
                        ]);
                        array_push($M3_3, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                                ...
                                if ($dataset_nutrient_template_data->count() > 0) {
                                    ...
                                    if (isset($item->{$datasetstring})) {
                                        ...
                                        if ($dataset_nutrient_templates->count() > 0) {
                                            ...
                                            $data['ppm'][$datasetstring] = number_format(($item->$datasetstring * $template->poly1) + $template->poly2, 1, '.', '');
                                        }
                                    }
                                }
                            }
...
$dataset = nutrient_template_data::where('nutriprobe',$request->node_address)->limit(1)->first();
                        if (isset($item->M3_1)) {
                            ...
                            $polynomials = nutrient_templates::where('id', $dataset->M3_1)->first();
                        $template = json_decode($polynomials->template);
                            $N03_avg += ($item->M3_1 * $template->poly1) + $template->poly2;
                        }
                        if (isset($item->M4_1)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M4_1)->first();
                        $template = json_decode($polynomials->template);
                            $N03_avg += (($item->M4_1 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M5_1)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M5_1)->first();
                        $template = json_decode($polynomials->template);
                            $N03_avg += (($item->M5_1 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M6_1)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M6_1)->first();
                        ...
                        if (isset($item->M3_3)) {
                            ...
                            $polynomials = nutrient_templates::where('id', $dataset->M3_3)->first();
                        $template = json_decode($polynomials->template);
                            $N03_avg += (($item->M3_3 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M4_3)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M4_3)->first();
                        $template = json_decode($polynomials->template);
                            $N03_avg += (($item->M4_3 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M5_3)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M5_3)->first();
                        $template = json_decode($polynomials->template);
                            $N03_avg += (($item->M5_3 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M6_3)) {
                            $count++;
                            $polynomials = nutrient_templates::where('id', $dataset->M6_3)->first();
                        $template = json_decode($polynomials->template);
                            $N03_avg += (($item->M6_3 * $template->poly1) + $template->poly2);
                        }
                        ...
                        if (isset($item->M3_2)) {
                            ...
                            $polynomials = nutrient_templates::where('id', $dataset->M3_2)->first();
                        $template = json_decode($polynomials->template);
                            $NH4_avg += ($item->M3_2 * $template->poly1) + $template->poly2;
                        }
                        if (isset($item->M4_2)) {
                            $count2++;
                            $polynomials = nutrient_templates::where('id', $dataset->M4_2)->first();
                        $template = json_decode($polynomials->template);
                            $NH4_avg += (($item->M4_2 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M5_2)) {
                            $count2++;
                            $polynomials = nutrient_templates::where('id', $dataset->M5_2)->first();
                        $template = json_decode($polynomials->template);
                            $NH4_avg += (($item->M5_2 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M6_2)) {
                            $count2++;
                            $polynomials = nutrient_templates::where('id', $dataset->M6_2)->first();
                        ...
                        if (isset($item->M3_4)) {
                            ...
                            $polynomials = nutrient_templates::where('id', $dataset->M3_4)->first();
                        $template = json_decode($polynomials->template);
                            $NH4_avg += (($item->M3_4 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M4_4)) {
                            $count2++;
                            $polynomials = nutrient_templates::where('id', $dataset->M4_4)->first();
                        $template = json_decode($polynomials->template);
                            $NH4_avg += (($item->M4_4 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M5_4)) {
                            $count2++;
                            $polynomials = nutrient_templates::where('id', $dataset->M5_4)->first();
                        $template = json_decode($polynomials->template);
                            $NH4_avg += (($item->M5_4 * $template->poly1) + $template->poly2);
                        }
                        if (isset($item->M6_4)) {
                            $count2++;
                            $polynomials = nutrient_templates::where('id', $dataset->M6_4)->first();
                        $template = json_decode($polynomials->template);
                            $NH4_avg += (($item->M6_4 * $template->poly1) + $template->poly2);
                        }

                        $NH4_avg = (($count2 > 0) ? ($NH4_avg / $count2) : 0);
*/
                        $N03_avg = $data['ppm']['M3_1'] + $data['ppm']['M4_1'] + $data['ppm']['M5_1'] + $data['ppm']['M6_1'] + $data['ppm']['M3_3'] + $data['ppm']['M4_3'] + $data['ppm']['M5_3'] + $data['ppm']['M6_3'];
                        $N03_avg = $N03_avg / 8;

                        $NH4_avg = $data['ppm']['M3_2'] + $data['ppm']['M4_2'] + $data['ppm']['M5_2'] + $data['ppm']['M6_2'] + $data['ppm']['M3_4'] + $data['ppm']['M4_4'] + $data['ppm']['M5_4'] + $data['ppm']['M6_4'];
                        $NH4_avg = $NH4_avg / 8;
                        array_push($data1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                ...
                if (!empty($field->nutrient_template_id) && $field->nutrient_template_id) {
                    ...
                    $row = nutri_data::where('node_address', $request->node_address)
                    ...
                    $gaugeN03_avg = $data['ppm']['M3_1'] + $data['ppm']['M4_1'] + $data['ppm']['M5_1'] + $data['ppm']['M6_1'] + $data['ppm']['M3_3'] + $data['ppm']['M4_3'] + $data['ppm']['M5_3'] + $data['ppm']['M6_3'];
                    $gaugeN03_avg = $gaugeN03_avg / 8;

                    $gaugeNH4_avg = $data['ppm']['M3_2'] + $data['ppm']['M4_2'] + $data['ppm']['M5_2'] + $data['ppm']['M6_2'] + $data['ppm']['M3_4'] + $data['ppm']['M4_4'] + $data['ppm']['M5_4'] + $data['ppm']['M6_4'];
                    $gaugeNH4_avg = $gaugeNH4_avg / 8;
...
*/
                    ...
                    $graph_data['nutrient_NO3']   = (float) number_format($gaugeN03_avg, 1, '.', '');
                    $graph_data['nutrient_NH4']   = (float) number_format($gaugeNH4_avg, 1, '.', '');
                }
            ...
            case 'nutrient_sm_avg':
                ...
                if (is_array($tpl_values)) {
                    ...
                    foreach ($nutr_data as $item) {
                        ...
                        $nutr_sm_avg = (($count > 0) ? ($nutr_sm_avg / $count) : 0);
            ...
            case 'nutrient_temp_avg':
                ...
                $plotOptions['series']['boostThreshold'] = 1;
                $plotOptions['series']['turboThreshold'] = 1;
                $plotOptions['series']['oum'] = $uom_symbol;
    ...
    public function nutrient_type($nutr_data, $request, $field)
    {
        ...
        if (!empty($field->nutrient_template_id) && $field->nutrient_template_id) {
            ...
            // $graph_data['nutrient_NO3']   = round($N03_avg, 1, PHP_ROUND_HALF_DOWN);
            $graph_data['nutrient_NO3']   = (float) number_format($N03_avg, 1, '.', '');;
            $graph_data['nutrient_NH4']   = round($NH4_avg, 1, PHP_ROUND_HALF_DOWN);
        }
    }
}
