...
class DataImportControllerHawk extends Controller
{
    ...
    public function dmtImportHawk(Request $request, $imei)
    {
        ...
        if (empty($request->no_raw_save)) {
            ...
            $raw = new raw_data_dmt_hawk();
            $raw->device_id = $imei;
            $raw->device_data = $json;
            $raw->save();
        ...
        $parse_aquacheck_string = function ($str) use ($moisture_probe_models, $nutrient_probe_models) {
            $d = [];
            $d['ven'] = strpos($str, '13AquaChck') !== false ? 'AquaChck' : 'DMT'; // probe vendor
            $str = str_replace('13AquaChck', '', $str);
            $str = str_replace(array_merge($moisture_probe_models, $nutrient_probe_models), '', $str);
            $d['ver'] = substr($str, 0, 3); // probe firmware version
            $d['ser'] = substr($str, 3); // probe serial
            return $d;
        };

        $array_find = function ($hs, array $nds) {
            foreach ($nds as $n) {
                if (stripos($hs, $n) !== false) {
                    return $n;
                }
            }
            return false;
        ...
        foreach ($data['Records'] as $row) {
            ...
            if (
                !empty($row['Fields'][0]['FType']) && $row['Fields'][0]['FType'] == 4
                && count($row['Fields']) < 11
            ) {
                continue;
            }
...
*/
            ...
            if (!empty($m_values) && !empty($t_values)) {
                ...
                foreach ($m_values as $key => $val) {
                    $item->{$key} = $val;
                }
                foreach ($t_values as $key => $val) {
                    $item->{$key} = $val;
                }
                ...
                $item->message_id_1 = "dmt";
                $item->message_id_2 = "dmt_{$node_address}_{$date_sampled}";
            ...
            if (!empty($n_values)) {
                ...
                $item->node_address = $node_address;
                $item->probe_serial = $probe_serial;
                $item->vendor_model_fw = $vendor_model_fw;
                $item->ver = $node_ver;
                foreach ($n_values as $key => $val) {
                    $item->{$key} = $val;
                }
                $item->date_reported = $date_reported;
                $item->date_sampled = $date_sampled;
