...
class CompanyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->initial) {
            $details = implode(
                ',',
                !empty($grants['Entities']['View']['C']) ? $grants['Entities']['View']['C'] : ($this->acc->is_admin ? ['All Objects'] : ['Access Denied'])
            );
    ...
    public function get(Request $request)
    {
        ...
        if (empty($request->id)) {
            return response()->json(['message' => 'missing id parameter']);
        }
        ...
        $cc = Company::where('id', $request->id)->first();
        if (!$cc) {
            return response()->json(['message' => 'company missing']);
        }
    ...
    public function add(Request $request)
    {
        ...
        $parent_comp = null;
        if ($request->parent) {
            $parent_comp = DB::table('distributors_companies')->insert([
                'user_id'           => $this->acc->id,         /* The Distributor User */
                'parent_company_id' => $request->parent, /* The Distributor User's Entity */
