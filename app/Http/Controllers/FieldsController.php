...
class FieldsController extends Controller
{
    ...
    public function manage_n(Request $request)
    {
        ...
        Log::debug('Get Node Config record');
        $hwconfig = hardware_config::where("node_address", $node_address)->first();
        if (!$hwconfig) {
            return response()->json(['message' => 'missing_node']);
        }

        Log::debug('Get Field associated with Node Config');
        ...
        if ($field) {
            ...
            if ($row) {
                ...
                // if ($count == 4 && $values) {
                Log::debug('date_reported');
                $dt = new \DateTime($row->date_reported);
                $tzObj = new \DateTimeZone($this->tz);
                $dt->setTimezone($tzObj);
                ...
                $data['soil_moist']['M0_1']    = number_format($row->M0_1, 2, '.', '');
                $data['soil_moist']['M0_2']    = number_format($row->M0_2, 2, '.', '');
                $data['soil_moist']['M0_3']    = number_format($row->M0_3, 2, '.', '');
                $data['soil_moist']['M0_4']    = number_format($row->M0_4, 2, '.', '');
                ...
                $data['temp']['M1_1']    = number_format($row->M1_1, 1, '.', '');
                $data['temp']['M1_2']    = number_format($row->M1_2, 1, '.', '');
                $data['temp']['M1_3']    = number_format($row->M1_3, 1, '.', '');
                $data['temp']['M1_4']    = number_format($row->M1_4, 1, '.', '');
                ...
                //     }
                // }
                // $data['ec']['M2_1']    = number_format($row->M2_1, 2, '.', '');
                // $data['ec']['M2_2']    = number_format($row->M2_2, 2, '.', '');
                // $data['ec']['M2_3']    = number_format($row->M2_3, 2, '.', '');
                // $data['ec']['M2_4']    = number_format($row->M2_4, 2, '.', '');
                $data['raw']['M3_1']    = number_format($row->M3_1, 1, '.', '');
                $data['raw']['M3_2']    = number_format($row->M3_2, 1, '.', '');
                $data['raw']['M3_3']    = number_format($row->M3_3, 1, '.', '');
                $data['raw']['M3_4']    = number_format($row->M3_4, 1, '.', '');
                $data['raw']['avg_M3'] = number_format(($data['raw']['M3_1'] + $data['raw']['M3_2'] + $data['raw']['M3_3'] + $data['raw']['M3_4']) / 4, 1);
                $data['raw']['M4_1']    = number_format($row->M4_1, 1, '.', '');
                $data['raw']['M4_2']    = number_format($row->M4_2, 1, '.', '');
                $data['raw']['M4_3']    = number_format($row->M4_3, 1, '.', '');
                $data['raw']['M4_4']    = number_format($row->M4_4, 1, '.', '');
                $data['raw']['avg_M4'] = number_format(($data['raw']['M4_1'] + $data['raw']['M4_2'] + $data['raw']['M4_3'] + $data['raw']['M4_4']) / 4, 1);
                $data['raw']['M5_1']    = number_format($row->M5_1, 1, '.', '');
                $data['raw']['M5_2']    = number_format($row->M5_2, 1, '.', '');
                $data['raw']['M5_3']    = number_format($row->M5_3, 1, '.', '');
                $data['raw']['M5_4']    = number_format($row->M5_4, 1, '.', '');
                $data['raw']['avg_M5'] = number_format(($data['raw']['M5_1'] + $data['raw']['M5_2'] + $data['raw']['M5_3'] + $data['raw']['M5_4']) / 4, 1);
                $data['raw']['M6_1']    = number_format($row->M6_1, 1, '.', '');
                $data['raw']['M6_2']    = number_format($row->M6_2, 1, '.', '');
                $data['raw']['M6_3']    = number_format($row->M6_3, 1, '.', '');
                $data['raw']['M6_4']    = number_format($row->M6_4, 1, '.', '');
                ...
                $data['ppm'][$datasetstring] = number_format(($row->$datasetstring * $template->poly1) + $template->poly2, 1, '.', '');
                }
                }
                }
                }
                }
                $data['pounds']['M3_1'] = number_format((($row->M3_1 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M3_2'] = number_format((($row->M3_2 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M3_3'] = number_format((($row->M3_3 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M3_4'] = number_format((($row->M3_4 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['avg_M3'] = number_format(($data['pounds']['M3_1'] + $data['pounds']['M3_2'] + $data['pounds']['M3_3'] + $data['pounds']['M3_4']) / 4, 1);
                $data['pounds']['M4_1'] = number_format((($row->M4_1 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M4_2'] = number_format((($row->M4_2 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M4_3'] = number_format((($row->M4_3 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M4_4'] = number_format((($row->M4_4 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['avg_M4'] = number_format(($data['pounds']['M4_1'] + $data['pounds']['M4_2'] + $data['pounds']['M4_3'] + $data['pounds']['M4_4']) / 4, 1);
                $data['pounds']['M5_1'] = number_format((($row->M5_1 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M5_2'] = number_format((($row->M5_2 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M5_3'] = number_format((($row->M5_3 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M5_4'] = number_format((($row->M5_4 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['avg_M5'] = number_format(($data['pounds']['M5_1'] + $data['pounds']['M5_2'] + $data['pounds']['M5_3'] + $data['pounds']['M5_4']) / 4, 1);
                $data['pounds']['M6_1'] = number_format((($row->M6_1 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M6_2'] = number_format((($row->M6_2 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M6_3'] = number_format((($row->M6_3 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['M6_4'] = number_format((($row->M6_4 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['pounds']['avg_M6'] = number_format(($data['pounds']['M6_1'] + $data['pounds']['M6_2'] + $data['pounds']['M6_3'] + $data['pounds']['M6_4']) / 4, 1);
                ////////////////////////////////end/////////
                $data['raw']['M4_1']    = number_format($row->M4_1, 1, '.', '');
                //   $data['ppm']['M4_1']    = number_format((($row->M4_1 * $poly1) + $poly2), 1, '.', '');
                $data['pounds']['M4_1'] = number_format((($row->M4_1 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['raw']['M5_1']    = number_format($row->M5_1, 1, '.', '');
                //   $data['ppm']['M5_1']    = number_format((($row->M5_1 * $poly1) + $poly2), 1, '.', '');
                $data['pounds']['M5_1'] = number_format((($row->M5_1 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                $data['raw']['M6_1']    = number_format($row->M6_1, 1, '.', '');
                //  $data['ppm']['M6_1']    = number_format((($row->M6_1 * $poly1) + $poly2), 1, '.', '');
                $data['pounds']['M6_1'] = number_format((($row->M6_1 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
                /*print_r($data) . PHP_EOL;
                print_r($row) . PHP_EOL;
                die;*/

                $raw_total = number_format($row->M3_1 + $row->M4_1 + $row->M5_1 + $row->M6_1, 1, '.', '');
                $ppm_total = number_format((($row->M3_1 * $poly1) + $poly2) + (($row->M4_1 * $poly1) + $poly2) + (($row->M5_1 * $poly1) + $poly2) + (($row->M6_1 * $poly1) + $poly2), 1, '.', '');
                $pounds_total = number_format((($row->M3_1 * $poly1) + $poly2) * $pound_conv + (($row->M4_1 * $poly1) + $poly2) * $pound_conv + (($row->M5_1 * $poly1) + $poly2) * $pound_conv + (($row->M6_1 * $poly1) + $poly2) * $pound_conv, 1, '.', '');
            }
           else
           {
               $values = null;
