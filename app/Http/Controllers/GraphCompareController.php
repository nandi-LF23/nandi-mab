...
class GraphController extends Controller
{
    ...
    public function sm_graph(Request $request)
    {
        ...
        $graph_type = !empty($graph_type) && $hwconfig->node_type == 'Nutrients' && !in_array($graph_type, ['nutrient', 'nutrient_ppm', 'nutrient_ppm_avg']) ? 'nutrient_ppm_avg' : $graph_type;
        ...
        $graph_type = empty($graph_type) && $hwconfig->node_type == 'Nutrients' ? 'nutrient_ppm_avg' : $graph_type;
        ...
        if (in_array($graph_type, ['sum', 'ave', 'temp', 'sm', 'stack', 'tech'])) {
            ...
            $node_data = $query->get();
        } else if (in_array($graph_type, ['nutrient', 'nutrient_ppm', 'nutrient_ppm_avg'])) {
        ...
        switch ($graph_type) {
                        ...
                        if ($stageCount >= 2) {
                            for ($i = 0; $i < $stageCount - 1; $i++) {
                                ...
                                if ($xVal >= $stages[$i]['x'] && $xVal <= $stages[$i + 1]['x']) {
                                    ...
                                    $rec['status'] = (float) ($capacity ? ((($rec['y'] - $stages[$i]['low']) / $capacity) * 100) : 0);
                                    $inAnyStage = true;
                                    break;
                                }
                            }
                            ...
                            if (!$inAnyStage) {
                                $capacity = $field->full - $field->refill;
                                $rec['status'] = (float) ($capacity ? ((($rec['y'] - $field->refill) / $capacity) * 100) : 0);
                            }
                        } else {
                            $rec['status'] = (float) ($capacity ? ((($rec['y'] - $field->refill) / $capacity) * 100) : 0);
                        }
            ...
            case 'sm':
                ...
                $getDefaultSMSeries = function () use ($uom_factor, $uom_suffix, $sm1, $sm2, $sm3, $sm4, $sm5, $sm6, $sm7, $sm8, $sm9, $sm10, $sm11, $sm12, $sm13, $sm14, $sm15, $ave) {
                    return [
                        ['name' => "S.M " . $uom_factor * 4  . $uom_suffix, 'type' => 'spline', 'data' => $sm1,  'dragDrop' => $grpDefs('grp1')],
                        ['name' => "S.M " . $uom_factor * 8  . $uom_suffix, 'type' => 'spline', 'data' => $sm2,  'dragDrop' => $grpDefs('grp2')],
                        ['name' => "S.M " . $uom_factor * 12 . $uom_suffix, 'type' => 'spline', 'data' => $sm3,  'dragDrop' => $grpDefs('grp3')],
                        ['name' => "S.M " . $uom_factor * 16 . $uom_suffix, 'type' => 'spline', 'data' => $sm4,  'dragDrop' => $grpDefs('grp4')],
                        ['name' => "S.M " . $uom_factor * 20 . $uom_suffix, 'type' => 'spline', 'data' => $sm5,  'dragDrop' => $grpDefs('grp5')],

                        ['name' => "S.M " . $uom_factor * 24 . $uom_suffix, 'type' => 'spline', 'data' => $sm6,  'dragDrop' => $grpDefs('grp6')],
                        ['name' => "S.M " . $uom_factor * 28 . $uom_suffix, 'type' => 'spline', 'data' => $sm7,  'dragDrop' => $grpDefs('grp7')],
                        ['name' => "S.M " . $uom_factor * 32 . $uom_suffix, 'type' => 'spline', 'data' => $sm8,  'dragDrop' => $grpDefs('grp8')],
                        ['name' => "S.M " . $uom_factor * 36 . $uom_suffix, 'type' => 'spline', 'data' => $sm9,  'dragDrop' => $grpDefs('grp9')],
                        ['name' => "S.M " . $uom_factor * 40 . $uom_suffix, 'type' => 'spline', 'data' => $sm10, 'dragDrop' => $grpDefs('grp10')],

                        ['name' => "S.M " . $uom_factor * 44 . $uom_suffix, 'type' => 'spline', 'data' => $sm11, 'dragDrop' => $grpDefs('grp11')],
                        ['name' => "S.M " . $uom_factor * 48 . $uom_suffix, 'type' => 'spline', 'data' => $sm12, 'dragDrop' => $grpDefs('grp12')],
                        ['name' => "S.M " . $uom_factor * 52 . $uom_suffix, 'type' => 'spline', 'data' => $sm13, 'dragDrop' => $grpDefs('grp13')],
                        ['name' => "S.M " . $uom_factor * 56 . $uom_suffix, 'type' => 'spline', 'data' => $sm14, 'dragDrop' => $grpDefs('grp14')],
                        ['name' => "S.M " . $uom_factor * 60 . $uom_suffix, 'type' => 'spline', 'data' => $sm15, 'dragDrop' => $grpDefs('grp15')],
...
*/
                        ...
                        array_push($M3_1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M3_1 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M3_2, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                        ...
                        array_push($M4_1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M4_1 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M4_2, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M4_2 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M4_3, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M4_3 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M4_4, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M4_4 * $poly1) + $poly2), 2, '.', ''),
                        ]);

                        array_push($M5_1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M5_1 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M5_2, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M5_2 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M5_3, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M5_3 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M5_4, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M5_4 * $poly1) + $poly2), 2, '.', ''),
                        ]);

                        array_push($M6_1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M6_1 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M6_2, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M6_2 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M6_3, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M6_3 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                        array_push($M6_4, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float)number_format((($item->M6_4 * $poly1) + $poly2), 2, '.', ''),
                        ]);
                    ...
                    $series[] = ['name' => 'M3.1', 'type' => 'spline', 'data' => $M3_1];
                    $series[] = ['name' => 'M3.2', 'type' => 'spline', 'data' => $M3_2];
                    $series[] = ['name' => 'M3.3', 'type' => 'spline', 'data' => $M3_3];
                    $series[] = ['name' => 'M3.4', 'type' => 'spline', 'data' => $M3_4];

                    $series[] = ['name' => 'M4.1', 'type' => 'spline', 'data' => $M4_1];
                    $series[] = ['name' => 'M4.2', 'type' => 'spline', 'data' => $M4_2];
                    $series[] = ['name' => 'M4.3', 'type' => 'spline', 'data' => $M4_3];
                    $series[] = ['name' => 'M4.4', 'type' => 'spline', 'data' => $M4_4];

                    $series[] = ['name' => 'M5.1', 'type' => 'spline', 'data' => $M5_1];
                    $series[] = ['name' => 'M5.2', 'type' => 'spline', 'data' => $M5_2];
                    $series[] = ['name' => 'M5.3', 'type' => 'spline', 'data' => $M5_3];
                    $series[] = ['name' => 'M5.4', 'type' => 'spline', 'data' => $M5_4];

                    $series[] = ['name' => 'M6.1', 'type' => 'spline', 'data' => $M6_1];
                    $series[] = ['name' => 'M6.2', 'type' => 'spline', 'data' => $M6_2];
                    $series[] = ['name' => 'M6.3', 'type' => 'spline', 'data' => $M6_3];
                    $series[] = ['name' => 'M6.4', 'type' => 'spline', 'data' => $M6_4];
                ...
                $graph_data = [
                    'graph' => array(
                        ...
                        'title' => [
                            'text' => $field->field_name . ' - Nutrients (PPM) - ' . $request->node_address,
                            'widthAdjust' => -200
                        ],
                        'plotOptions' => $plotOptions
                    ),
                ...
                if (!empty($field->nutrient_template_id) && $field->nutrient_template_id) {
                    ...
                    $graph_data['nutrient_NH4']   = $results['NH4_avg'];
                    $graph_data['nutrient_label'] = $results['nutrient_label'];
                }
                die;
...
*/
                        ...
                        array_push($data1, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float) number_format($M3_avg_val, 2, '.', ''),
                        ]);
                        array_push($data2, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float) number_format($M4_avg_val, 2, '.', ''),
                        ]);
                        array_push($data3, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float) number_format($M5_avg_val, 2, '.', ''),
                        ]);
                        array_push($data4, [
                            ($dt->getTimestamp() + $dt->getOffset()) * 1000,
                            (float) number_format($M6_avg_val, 2, '.', ''),
                        ]);
                    }
                }
