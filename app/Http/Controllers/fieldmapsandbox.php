    ...
    public function index(Request $request)
    {
        ...
        foreach ($nodes as $index => &$node) {
            ...
            if ($node['node_type'] == 'Soil Moisture') {
                ...
                // $rows2 = hardware_config::select([
                //   'hardware_config.*',
                //   DB::raw('UNIX_TIMESTAMP(updated_at) as sort_date')
                // ])
                ...
                //   DB::raw('UNIX_TIMESTAMP(date_time) as sort_date')
                // ])
                // ->where('probe_id', '0x000d1208Test-0')
                // ->where('average', '>', 0)
                // ->where('accumulative', '>', 0)
                //   ->orderBy('id', 'desc')
                //   ->limit(2)
                ...
                // if ($rows->count() == null) {
                //   dd('no readings in node_data');
                // }else if ($rows->count() == 2) {
                //   dd('more than 0 in node_data');
                // }

                // $rows = hardware_config::select([
                //   'hardware_config.*',
                //   DB::raw('UNIX_TIMESTAMP(updated_at) as sort_date')
                // ])->where('node_address', $node_address)->get();

                //     dd(hardware_config::select(
                //     'hardware_config.updated_at',
                // )->where('node_address', '354679096315711-2')->get());
                ...
                // else if ($rows->count() == null) {
                //   $data = $rows2[0]->toArray(); // newest data row

                // $hw_c = hardware_config::select(['hardware_config.updated_at'])->where('node_address', $node_address)->get();
                // dd($hw_c);
                // }
                // dd($data['id']);
                // dd('problem');
