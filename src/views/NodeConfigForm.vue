    ...
    <div class='container-fluid mt--6'>
      ...
      <b-modal v-model="showUploadPerimeterModal" centered>
              ...
              <b-row v-if="upload_model.progress">
                <b-col md>
                  <b-progress :max="100" height="2rem">
                    <b-progress-bar :value="upload_model.progress" :label="upload_model.progress + '%'" :variant="upload_model.variant" :animated="!upload_model.complete"></b-progress-bar>
                  </b-progress>
                </b-col>
              </b-row>
            </form>
        ...
        <template #modal-footer="{ ok, cancel, hide }">
            ...
            <base-button
              type="primary"
              @click="upload_model.complete ? closeUploadPerimeterModal() : uploadPerimeterFiles()" :disabled="upload_model.busy && !upload_model.complete">
      ...
      <b-modal v-model="showSetPerimeterModal" centered>
        ...
        <template #default="{ hide }">
          <b-row>
            <b-col>
              <base-input label=" ">
                <MapPolyDrawer
                  ...
                  modalTitle="Draw Perimeter"
                  buttonTitle="Draw Perimeter"
                  :startPos="startPos"
                  :polyData="model.perimeter"
                  :additionalLayers="additional_layers"
                  @savePolyData="savePerimeter"
            ...
            <b-col>
              <base-input label=" ">
                <b-button variant="outline-primary" :block="true" @click="openUploadPerimeterModal">Upload Zone File</b-button>
              </base-input>
            </b-col>
          </b-row>
          <b-row>
            <b-col>
              <base-input label=" ">
                <b-button variant="outline-primary" :block="true" @click="confirmPerimeterClearance">Clear Perimeter</b-button>
              </base-input>
            </b-col>
            <b-col>
              <base-input label=" ">
                <b-button variant="outline-primary" :block="true" @click="confirmFieldZonesClearance">Clear Zones</b-button>
              </base-input>
            </b-col>
          </b-row>
        </template>
                ...
                <b-col>
                  <div>
                    <base-input
                      ...
                      autocomplete="off">
                      <el-select
                        :disabled="!model.node_type || Object.values(companies).length < 2"
                        @change="companyChanged"
                        v-model="model.company_id"
                        filterable
                        placeholder="Entity"
                        v-b-tooltip.hover.top title="Change the node's owning entity. Note: After changing the entity, you have to also choose a new device before the changes would take effect."
                        autocomplete="off">
                  ...
                  <div>
                    <base-input label="Device Type" name="device type" rules="required" vid="hardware_management_id">
                      <el-select v-model="model.hardware_management_id">
                        <el-option v-for="device in device_types" :key="device.id" :label="device.device_make" :value="device.id">
                          <span style="float: left">{{ device.device_make }}</span>
                          <span style="float: right; color: #8492a6; font-size: 13px; margin-right:1em">{{ device.company_name }}</span>
                        </el-option>
                      </el-select>
                    </base-input>
                  </div>
                  ...
                  <div>
                    <base-input label="Node Type" name="node type" rules="required" vid="node_type">
                      <el-select @change="nodeTypeChanged" v-model="model.node_type" filterable placeholder="Node Type">
                        ...
                        <el-option :disabled="!userCan('Add', 'Node Config') && !userCan('Add', 'Nutrients')"     label="Nutrients" value="Nutrients"></el-option>
                        <el-option :disabled="!userCan('Add', 'Node Config') && !userCan('Add', 'Well Controls')" label="Well Controls" value="Wells"></el-option>
                        <el-option :disabled="!userCan('Add', 'Node Config') && !userCan('Add', 'Meters')"        label="Water Meter" value="Water Meter"></el-option>
                      </el-select>
                    </base-input>
                  </div>
                </b-col>
          ...
          <validation-observer ref='change_address_form' slim>
            <form role="form" @submit.prevent="()=>false">
              <b-row>
                <b-col>
                  <div>
                    <base-input label="Node Address" vid="node_address" rules="required" name="node address" placeholder="Node Address" v-model="node_address_edit"></base-input>
                  </div>
                </b-col>

                <b-col>
                  <div>
                    <base-input label="Probe Address"  vid="probe_address" rules="required|integer" name="probe_address" placeholder="Probe Address" v-model="probe_address_edit"></base-input>
                  </div>
                </b-col>
              </b-row>
            </form>
      ...
      <validation-observer ref='form' slim>
        <div class='row'>
            ...
            <card body-classes="px-0 py-0">
              <template slot="header">
                ...
                <base-button v-if="model.node_type == 'Soil Moisture' || model.node_type == 'Nutrients'"
                  :disabled="!userCan('Edit', 'Node Config') || loading"
                  @click.native="openSetPerimeterModal"
                  class="btn"
                  size="sm"
                  type="primary"
                  icon>
              ...
              <div class="card-body">
                  ...
                  <b-col md>
                    <div>
                      <base-input label="Node Address">
                        <b-input-group>
                          <b-form-input type="text" v-model="model.node_address" placeholder="Node Address" readonly></b-form-input>
                          <b-input-group-append>
                            <b-button :disabled="!isDistributor() && !isAdmin()" variant="outline-primary" @click="openAddressChangeModal">Change</b-button>
                          </b-input-group-append>
                        </b-input-group>
                      </base-input>
                    </div>
                  ...
                  <b-col md>
                    <div>
                      <base-input label="Node Type">
                        <b-input-group>
                          <b-form-input type="text" v-model="model.node_type" placeholder="Node Type" readonly></b-form-input>
                          <b-input-group-append>
                            <b-button :disabled="!isDistributor() && !isAdmin()" variant="outline-primary" @click="openNodeTypeChangeModal">Change</b-button>
                          </b-input-group-append>
                        </b-input-group>
                      </base-input>
                    </div>
                ...
                <b-row align-v="center">
                  <b-col md>
                    <div>
                      <base-input @change="syncFields" name="latitude" :rules="{ regex:/^-?\d+(\.\d{1,15})?$/ }" label="Latitude" placeholder="Latitude" vid="latt" v-model="model.latt"></base-input>
                    </div>
                  </b-col>
                  <b-col md>
                    <div>
                      <base-input @change="syncFields" name="longitude" :rules="{ regex:/^-?\d+(\.\d{1,15})?$/ }" label="Longitude" placeholder="Longitude" vid="lng" v-model="model.lng"></base-input>
                    </div>
                  </b-col>
                  <b-col md>
                    <div>
                      <base-input label=" ">
                        <MapCoordsPicker
                          ...
                          buttonTitle="Set Coords Visually"
                          :additionalLayers="additional_layers"
                          :startPos="startPos"
                          @setCoords="setCoords">
                        </MapCoordsPicker>
                  ...
                  <b-col md>
                    <div>
                      <base-input
                        label=" "
                        v-b-tooltip.hover.top title="Locking the coordinates will prevent them from being overridden by device updates.">
                        <b-form-checkbox size="lg" @change="syncFields" v-model="model.coords_locked" :unchecked-value="0" :value="1">
                          Lock Coordinates?
                        </b-form-checkbox>
                      </base-input>
                    </div>
                  ...
                  <b-col md>
                    <div>
                      <base-input
                        ...
                        vid="wl_station_name"
                        v-b-tooltip.hover.top title="The node's serial number"
                  ...
                  <b-col md>
                    <div>
                      <base-input
                        ...
                        vid="wl_station_name"
                        v-b-tooltip.hover.top title="Optionally enter an associated weather station's name."
                        v-model="model.wl_station_name">
                      </base-input>
                    </div>
                  </b-col>
                  <b-col md>
                    <div>
                      <base-input label="Entity">
                        <b-input-group>
                          <b-form-input type="text" v-model="node_company_name" placeholder="Entity" readonly></b-form-input>
                          <b-input-group-append>
                            <b-button :disabled="!isDistributor() && !isAdmin()" variant="outline-primary" @click="openNodeEntityChangeModal">Change</b-button>
                          </b-input-group-append>
                        </b-input-group>
                      </base-input>
                    </div>
                  ...
                  <b-col cols="6" sm="4">
                    <div>
                      <base-input 
                        ...
                        rules="integer"
                        label="Field Zone ID"
                        placeholder="Field Zone ID"
                        vid="zone" v-model="model.zone">
                      </base-input>
                    </div>
                  </b-col>
                </b-row>
            ...
            <card body-classes="px-0 py-0">
              ...
              <div class="card-body">
                <b-row align-v="center">
                  ...
                  <b-col md>
                    <div>
                      <base-input @change="syncFields" name="device serial number" rules="required" label="Device Serial Number" placeholder="Device Serial" vid="device_serial_number"  v-model="model.device_serial_number"></base-input>
                    </div>
                  </b-col>
                </b-row>
              </div>
          ...
          <div class='col-md-12' v-if="model.integration_opts && Object.keys(model.integration_opts).length && userCan('Integrate', 'Entities')">
            <card body-classes="px-0 py-0">
              <template slot="header">
              ...
              <div class="card-body">
                <template v-for="(opts, slug) in model.integration_opts">
                  <b-row :key="slug">
                    <b-col><h4>{{ slug }} Options</h4></b-col>
                  </b-row>
                  <b-row>
...
export default {
  ...
  methods: {
    ...
    {
      this.loading = true;
      this.$axios.get(this.$store.state.baseUrl+"/api/hardwareconfig/" + this.$route.params.node_address)
      .then((resp) => {
        this.loading = false;
    ...
    syncFields()
    {
      ...
      .then(success => {
        ...
        this.loading = true;
        return this.$axios.post(this.$store.state.baseUrl+"/api/hardwareconfigsave", this.model )
        .then((response) => {
          this.loading = false;
    ...
    changeNodeAddress()
    {
      this.$refs.change_address_form.validate().then(success => {
        ...
        this.loading = true;
        this.$axios.post(this.$store.state.baseUrl+"/api/hardwareconfigaddrchange", 
          { 
    ...
    clearFieldPerimeter(field_id){
      ...
      this.loading = true;
      this.$axios.post(this.$store.state.baseUrl+"/api/clearfieldperimeter", {
        field_id: field_id 
      }).then((response) => {
        this.loading = false;
        if(response.data.message == 'success'){
    ...
    clearFieldZones(field_id){
      ...
      this.loading = true;
      this.$axios.post(this.$store.state.baseUrl+"/api/clearfieldzones", {
        field_id: field_id 
      }).then((response) => {
        this.loading = false;
        if(response.data.message == 'success'){
