        ...
        <b-row>
          <b-col lg="4" md="6">
            <div v-if="nodeFilterType == 'nut' && field_model.node_type == 'nut'">
              <div class='text-center'>PPM Avg</div>
              <NiftyGauge
                :angle="field_model.nutrient_gauge"
                :tooltip="'L: ' + field_model.nutrient_lower + ', U: ' + field_model.nutrient_upper + ', A: ' + field_model.nutrient_avg"
                :size="'50%'"
                :subl="field_model.nutrient_lower"
                :subm="field_model.nutrient_avg"
                :subr="field_model.nutrient_upper"
                :label="field_model.nutrient_label"
          ...
          <b-col lg="4" sm="6">
            <div v-if="nodeFilterType == 'sm' && (field_model.node_type == 'sm' || field_model.node_type == 'nut')">
              ...
              <NiftyGauge
                :angle="field_model.temp_gauge"
                :size="'50%'"
                :subl="'0'"
                :subm="field_model.temp_avg"
                :subr="'100'"
      ...
      <template v-for="(marker, idx) in markers">
        ...
        <template v-if="marker.zones && marker.zones.length">
          <MglGeojsonLayer v-for="(zone, idx2) in marker.zones" :key="idx+'_'+idx2"
            :sourceId="zone.layer.id"
            :layerId="zone.layer.id"
            :source="zone.source"
            :layer="zone.layer"
            @click="setFieldModalZoneInfo(marker.node_address, zone)"
            :clearSource="false"/>
        </template>

        <MglGeojsonLayer v-if="marker.perimeter"
          :sourceId="marker.node_address"
          :layerId="marker.node_address"
          :source="marker.perimeter"
          :layer="marker.layer"
          :clearSource="false"
          @click="showFieldModal(marker.node_address)"
          @mouseover="showLayerCursor"
          @mouseout="hideLayerCursor"/>

        <MglMarker 
          :key="marker.node_address+'_'+idx"
          :ref="marker.node_address"
          @click="!marker.perimeter ? showFieldModal(marker.node_address) : {}"
