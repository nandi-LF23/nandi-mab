    ...
    <div class='container-fluid mt--6'>
      ...
      <b-modal ref="my-modal" v-model="browseTemplatesModalVisible" centered no-close-on-esc no-close-on-backdrop
              ...
              <template #default="{ hide }">
                <b-row>
                  ...
                  <b-col>
                    ...
                    <b-col class="horiz-avg">
                      <b-toast v-if="toggle == 1" :visible="true" title="Avg" static no-auto-hide no-close-button>
                        {{ horizontal1_avg }}
                      </b-toast>
                      <b-toast v-else :visible="false" title="Avg" static no-auto-hide no-close-button>
                        &nbsp;
                      </b-toast>
                    </b-col>
                </b-row>
                ...
                <b-row>
                  ...
                  <b-col class="horiz-avg">
                    ...
                    <b-toast v-if="toggle == 1" :visible="false" title="Avg" static no-auto-hide no-close-button>
                      &nbsp;
                    </b-toast>
                  </b-col>
                </b-row>
                <b-row>
                  ...
                  <b-col class="horiz-avg">
                    <b-toast v-if="toggle == 1" :visible="true" title="H. Avg" static no-auto-hide no-close-button>
                      {{ horizontal4_avg }}
                    </b-toast>
                    <b-toast v-else :visible="false" title="Avg" static no-auto-hide no-close-button>
                      &nbsp;
                ...
                <b-row v-if="toggle == 2" class="vert-avg">
                  <b-col>
                    <b-toast :visible="true" title="Average" static no-auto-hide no-close-button>
                      {{ col1_avg }}
                    </b-toast>
                  </b-col>
                        ...
                        <b-list-group>
                          <b-list-group-item class="p-0 pl-2"><span class="sensor_type_heading">M3.1</span>
                            <b-form-select @change.native="saveSensorSetup" class="sensor_select"
                              ...
                              <b-form-select-option v-for="item in loadedTemplates" :value="item.id">{{
                                item.name
                ...
                <b-row>
                  <b-col class='text-center'>
                    <div class="d-flex flow-row justify-content-center">
                      <div class="m-1">
                        Presently set to: {{ (toggle == 1) ? "Horizontal" : "Vertical" }}<br /><br />
                        <b-button @click="changeGroup" class="ml-auto">{{ (toggle == 1) ? "Vertical" : "Horizontal" }}
                        </b-button>
        ...
        <template #modal-footer="{ ok, cancel, hide }">
          ...
          <b-button variant="outline-primary" @click="closeBrowseTemplatesModal(); saveSensorSetup();">
            Close
          </b-button>
        </template>
...
export default {
  ...
  computed: {
    ...
    m6ChartOptions() {
      var temperature = parseFloat(this.soil_temp);
  ...
  methods: {
    ...
    /*
      <b-form-select :options="options1" v-model="mySelected1" @change.native="changeGroup(1, mySelected1)">
                        </b-form-select>  <b-form-select :options="options1" v-model="mySelected1" @change.native="changeGroup(1, mySelected1)">
                        </b-form-select>
    */
    changeGroup() {
      console.log(this.nutrient_groups);
      console.log('in set');
      if (
        (this.nutrient_groups.M3_1 == 1) &&
        (this.nutrient_groups.M4_1 == 1) &&
      ...
      this.$axios.get("/api/ManageNutrients/" + this.$route.params.node_address)
        .then((resp) => {
          ...
          if (resp.data) {
            ...
            this.col6_avg = (this.col6_avg / 4).toFixed(1);
            ...
            this.soil_m = (this.soil_m / 4).toFixed(1);
            ...
            this.soil_temp = (this.soil_temp / 4).toFixed(1);
