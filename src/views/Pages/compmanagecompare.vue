    ...
    <div class="content">
        ...
        <b-container fluid class="mt--6">
            ...
            <b-modal v-model="showCompanyCreationModal" size='md' centered no-close-on-esc no-close-on-backdrop>
                ...
                <template #default="{ hide }">
                    <el-steps :active="formStepIndex" finish-status="success" align-center>
                        <!-- <el-step title="Entity Type"></el-step> -->
                        <el-step title="Entity Type"></el-step>
            ...
            <el-step title="Groups (Optional)" v-if="isAdmin()"></el-step> -->
                    ...
                    <validation-observer ref='addstep_enttype' slim>
                        <form role="addstep_enttype" v-show="formStep == 'enttype'" @submit.prevent="() => false">

                            <b-row><b-col>
                                    <h3>Select if Parent or Child Entity</h3>
                                </b-col></b-row>

                            <b-row align-v="center">
                                <b-col md>
                                    <base-input name="entity type" rules="required">
                                        <el-select v-model="entity_select.value" placeholder="Select">
                                            <el-option v-for="item in entity_select" :value="item.value" :key="item.value"
                                                :label="item.label"></el-option>
                                        </el-select>
                                    </base-input>
                                    <base-input v-if="entity_select.value == 'Child'" name="entity" rules="required">
                                        <el-select v-model="tableData.company_name" value-key="id" filterable
                                            placeholder="Select">
                                            <el-option v-for="item in tableData" v-if="item.is_distributor == 0"
                                                :key="item.company_name" :label="item.company_name"
                                                :value="item.id"></el-option>
                                        </el-select>
                                    </base-input>
                                    <base-input v-else name="entity">
                                        <el-select placeholder="Select" disabled>
                                            <el-option></el-option>
                                        </el-select>
                                    </base-input>
                                </b-col>
                    ...
                    <validation-observer ref='addstep_basic' slim>
                        <form role="addstep_basic" v-show="formStep == 'basic'" @submit.prevent="() => false">
                            <b-row>
                                <b-col md>
                                    <h3>Basic Info</h3>
                                    <base-input label="Entity Name" name="entity name" rules="required"
                                        placeholder="Entity Name" vid="company_name"
                                        v-model="model.company_name"></base-input>
                                    <base-input label="Entity Logo" name="entity logo">
                                        <ImageCropUpload
                                            :stencilProps="{ scalable: true, movable: true/*, aspectRatio: 2/1*/ }"
                                            :resize="{ height: 32, width: 'auto' }" @clear="clearLogo" @done="setLogo" />
                                    </base-input>
                                    <base-input label="Contact Name" name="contact name" placeholder="Contact Name"
                                        vid="contact_name" v-model="model.contact_name" rules="required">
                                    </base-input>
                                    <base-input label="Contact Email" name="contact email" placeholder="Contact Email"
                                        vid="contact_email" v-model="model.contact_email"></base-input>
                                    <base-input label="Contact Phone" name="contact phone" placeholder="Contact Phone"
                                        vid="contact_phone" v-model="model.contact_phone"></base-input>
                                </b-col>
                            </b-row>
                        </form>
                    </validation-observer>
                        ...
                        <form role="addstep_paddr" v-show="formStep == 'paddr'" @submit.prevent="() => false">
                            <b-row>
                                <b-col md>
                                    ...
                                    <base-input name="physical address line 1" placeholder="Physical Address Line 1"
                                        vid="physical_address.line_1" v-model="model.physical_address.line_1"></base-input>
                                    <base-input name="physical address line 2" placeholder="Physical Address Line 2"
                                        vid="physical_address.line_2" v-model="model.physical_address.line_2"></base-input>
                                    <base-input name="physical city" placeholder="Physical Town/City"
                                        vid="physical_address.city" v-model="model.physical_address.city"></base-input>
                                    <base-input name="physical postal code" placeholder="Physical Postal Code"
                                        vid="physical_address.postalcode"
                                        v-model="model.physical_address.postalcode"></base-input>

                                    <base-input name="physical country" placeholder="Physical Country"
                                        vid="physical_address.country">
                                        <el-select v-model="model.physical_address.country" filterable
                                            placeholder="Country">
                        ...
                        <form role="addstep_baddr" v-show="formStep == 'baddr'" @submit.prevent="() => false">
                            <b-row>
                                <b-col md>
                                    ...
                                    <base-input>
                                        <b-form-checkbox v-model="model.p_as_b" name="p_as_b" value="yes"
                                            unchecked-value="no">
                                            Same as Physical?
                                        </b-form-checkbox>
                                    </base-input>
                                    <base-input name="billing address line 1" placeholder="Billing Address Line 1"
                                        vid="billing_address.line_1"
                                        v-model="model.p_as_b == 'yes' ? model.physical_address.line_1 : model.billing_address.line_1"
                                        :disabled="model.p_as_b == 'yes'"></base-input>
                                    <base-input name="billing address line 2" placeholder="Billing Address Line 2"
                                        vid="billing_address.line_2"
                                        v-model="model.p_as_b == 'yes' ? model.physical_address.line_2 : model.billing_address.line_2"
                                        :disabled="model.p_as_b == 'yes'"></base-input>
                                    <base-input name="billing city" placeholder="Billing Town/City"
                                        vid="billing_address.city"
                                        v-model="model.p_as_b == 'yes' ? model.physical_address.city : model.billing_address.city"
                                        :disabled="model.p_as_b == 'yes'"></base-input>
                                    <base-input name="billing postal code" placeholder="Billing Postal Code"
                                        vid="billing_address.postalcode"
                                        v-model="model.p_as_b == 'yes' ? model.physical_address.postalcode : model.billing_address.postalcode"
                                        :disabled="model.p_as_b == 'yes'"></base-input>

                                    <base-input name="billing country" placeholder="Billing Country"
                                        vid="billing_address.country">
                                        <el-select
                                            v-model="model.p_as_b == 'yes' ? model.physical_address.country : model.billing_address.country"
                                            :disabled="model.p_as_b == 'yes'" filterable placeholder="Country">
                                            <el-option v-for="item in countries" :key="item.code" :label="item.name"
                                                :value="item.code">
                                            </el-option>
                                        </el-select>
                            ...
                            <b-row>
                                <b-col md>
                                    ...
                                    <base-input name="modules" vid="modules" rules="required">
                                        <b-form-checkbox size="lg" v-model="model.modules" unchecked-value=""
                                            value="Soil Moisture">Soil
                                            Moisture</b-form-checkbox>
                                        <b-form-checkbox v-if="isAdmin()" size="lg" v-model="model.modules"
                                            unchecked-value="" value="Nutrients">Nutrients</b-form-checkbox>
                                        <b-form-checkbox size="lg" v-model="model.modules" unchecked-value=""
                                            value="Well Controls">Well
                                            Controls</b-form-checkbox>
                                        <b-form-checkbox size="lg" v-model="model.modules" unchecked-value=""
                                            value="Meters">Meters</b-form-checkbox>
                                    </base-input>
                                </b-col>
                            </b-row>
                        </form>
                    ...
                    <b-row v-show="formStep == 'confirm'">
                        <b-col>
                            ...
                            <b-row>
                                <b-col>
                                    <b-table-simple responsive small>
                                        <b-tr><b-td><strong>Entity
                                                    Name</strong></b-td><b-td><strong>Logo?</strong></b-td></b-tr>
                                        <b-tr><b-td>{{ model.company_name }}</b-td><b-td>{{ model.company_logo ? 'Yes' :
                                            'No' }}</b-td></b-tr>
                                    </b-table-simple>
                                </b-col>
                            </b-row>
                            ...
                            <b-row>
                                <b-col>
                                    <b-table-simple responsive small>
                                        <b-tr><b-td><strong>Contact Name</strong></b-td><b-td><strong>Contact
                                                    Email</strong></b-td><b-td><strong>Contact Phone</strong></b-td></b-tr>
                                        <b-tr><b-td>{{ model.contact_name }}</b-td><b-td>{{ model.contact_email
                                        }}</b-td><b-td>{{
    model.contact_phone }}</b-td></b-tr>
                                    </b-table-simple>
                                </b-col>
                            </b-row>
                            ...
                            <b-row>
                                <b-col>
                                    <b-table-simple responsive small>
                                        <b-tr><b-td><strong>Address
                                                    Details</strong></b-td><b-td><strong>Physical</strong></b-td><b-td><strong>Billing</strong></b-td></b-tr>
                                        <b-tr><b-td>Line 1</b-td><b-td>{{ model.physical_address.line_1 }}</b-td><b-td>{{
                                            model.p_as_b ==
                                            'yes' ? model.physical_address.line_1 : model.billing_address.line_1
                                        }}</b-td></b-tr>
                                        <b-tr><b-td>Line 2</b-td><b-td>{{ model.physical_address.line_2 }}</b-td><b-td>{{
                                            model.p_as_b ==
                                            'yes' ? model.physical_address.line_2 : model.billing_address.line_2
                                        }}</b-td></b-tr>
                                        <b-tr><b-td>City/Town</b-td><b-td>{{ model.physical_address.city }}</b-td><b-td>{{
                                            model.p_as_b ==
                                            'yes' ? model.physical_address.city : model.billing_address.city
                                        }}</b-td></b-tr>
                                        <b-tr><b-td>Postal Code</b-td><b-td>{{ model.physical_address.postalcode
                                        }}</b-td><b-td>{{
    model.p_as_b == 'yes' ? model.physical_address.postalcode :
    model.billing_address.postalcode
}}</b-td></b-tr>
                                        <b-tr><b-td>Country</b-td><b-td>{{ model.physical_address.country }}</b-td><b-td>{{
                                            model.p_as_b ==
                                            'yes' ? model.physical_address.country : model.billing_address.country
                                        }}</b-td></b-tr>
                                    </b-table-simple>
                                </b-col>
                            </b-row>
                ...
                <b-col md v-if='model.add_groups.length'>
                  <b-table-simple responsive small>
                    ...
                    <b-tr><b-td><span class='wswrap' v-if="subsystems">{{ model.add_groups.map(i => i.group_name + ' (' +
                      subsystems[i.subsystem_id].subsystem_name + ')').join(', ') }}</span></b-td></b-tr>
                  </b-table-simple>
                </b-col>
              </b-row> -->
                ...
                <template #modal-footer="{ ok, cancel, hide }">
                    <base-button type="outline-primary" class="mr-auto"
                        @click="closeCreateCompanyModal()">Cancel</base-button>
                    <base-button type="outline-primary" class="ml-auto" @click="prevStep()"
                        v-show="formStep != 'enttype'">Previous</base-button>
                    <base-button type="primary" @click="formStep == 'confirm' ? createCompany() : nextStep()"> {{ formStep
                        ==
            ...
            <b-modal v-model="showCompanyRemovalModal" size='md' centered no-close-on-esc no-close-on-backdrop>
                ...
                <template #modal-header="{ close }">
                    <h6 slot="header" class="modal-title" id="modal-title-default">Remove Entity: {{ delete_company_name }}
                    </h6>
                </template>
                ...
                <template #default="{ hide }">
                    <validation-observer ref='remcc' v-slots="{ validate, reset }" slim>
                        <form class='cc_move_form' role="form" @submit.prevent="() => false">
                            <b-row>
                                <b-col md>
                                    <h2>{{ move_model.move_company ? 'Relocation Configuration' : 'Please confirm Entity
                                                                            Removal' }} </h2>
                                </b-col>
                            </b-row>
                            <b-row>
                                <b-col md>
                                    <!-- Move Entity Option -->
                                    <b-form-checkbox size="lg" v-model="move_model.move_company" name="move_company"
                            ...
                            <template v-if="move_model.move_company">
                                <b-row>
                                    <b-col md>
                                        ...
                                        <base-input label="Destination Entity" rules="required" vid="destination_entity"
                                            name="destination entity">
                                            <el-select @change="getNewCompanyMoveMeta(move_model.new_company_id)"
                                                v-model="move_model.new_company_id" filterable value-key="id"
                                                placeholder="Choose Entity">
                                                <template v-for="(cc, index) in move_model.target_companies">
                                                    <el-option :key="index" :label="cc.company_name"
                                                        :value="cc.id"></el-option>
                                                </template>
                                            </el-select>
                                        </base-input>
                                    </b-col>
                                ...
                                <b-row v-if="move_model.new_company_id">
                                    <b-col md>
                                        <base-input label="Choose Objects" rules="required" vid="move_types"
                                            name="chosen objects">
                                            <b-form-group>
                                                <b-form-checkbox-group @change="reset" size="lg"
                                                    v-model="move_model.move_types" :options="move_model.movable_objects">
                                    ...
                                    <b-col md>
                                        ...
                                        <b-table-simple responsive small
                                            ...
                                            <b-tr>
                                                ...
                                                <b-th class='text-right'><strong>{{
                                                    move_model.target_companies['"' +
                                                        move_model.new_company_id.toString() + '"'].company_name
                                                }}</strong></b-th>
                                            </b-tr>
                                            <template v-for="(sensormap, index) in move_model.sensor_mappings">
                                                <b-tr :key="sensormap.old_sensor_id">
                                                    <template v-if="move_model.counts[sensormap.old_sensor_type]">
                                                        ...
                                                        <b-td class='valign'>
                                                            <base-input :vid="'new_sensors_' + index" rules="required"
                                                                name="new sensor choice">
                                                                <el-select v-model="sensormap.new_sensor_id" value-key="id"
                                                                    size="small" filterable rules="required"
                                                                    placeholder="New Sensor">
                                                                    <el-option v-for="sensor in move_model.new_sensors"
                                                                        v-if="sensormap.old_sensor_type == sensor.device_type"
                                                                        :key="sensor.id" :label="sensor.device_make"
                                                                        :value="sensor.id">
                                                                    </el-option>
                                                                </el-select>
                                                            </base-input>
                                        ...
                                        <p v-else>
                                            ...
                                            <template v-else-if="move_model.new_sensors.length == 0">
                                                <strong>{{ move_model.target_companies['"' +
                                                    move_model.new_company_id.toString() +
                                                    '"'].company_name
                                                }}</strong> has no sensors defined.<br>
                                ...
                                <b-row v-if="move_model.new_company_id && move_model.move_types.indexOf('users') !== -1">
                                    <b-col md>
                                        ...
                                        <b-table-simple responsive small
                                            v-if="move_model.role_mappings.length > 0 && move_model.new_roles.length > 0">
                                            <b-tr>
                                                <b-th>{{ delete_company_name }}</b-th>
                                                <b-th></b-th>
                                                <b-th class='text-right'>{{
                                                    move_model.target_companies['"' + move_model.new_company_id.toString() +
                                                        '"'].company_name
                                                }}</b-th>
                                            </b-tr>
                                            <template v-for="(rolemap, index) in move_model.role_mappings">
                                                <b-tr :key="rolemap.old_role_id">
                                                    ...
                                                    <b-td class='valign'>
                                                        <base-input :vid="'new_roles_' + index" rules="required"
                                                            name="new role choice">
                                                            <el-select v-model="rolemap.new_role_id" value-key="id"
                                                                size="mini" filterable rules="required"
                                                                placeholder="New Role">
                                                                <el-option v-for="role in move_model.new_roles"
                                                                    :key="role.id" :label="role.role_name" :value="role.id">
                                                                </el-option>
                                                            </el-select>
                                                        </base-input>
                                                    </b-td>
                                        ...
                                        <p v-else>
                                            ...
                                            <template v-else-if="move_model.new_roles.length == 0">
                                                <strong>{{ move_model.target_companies['"' +
                                                    move_model.new_company_id.toString() +
                                                    '"'].company_name
                                                }}</strong> has no roles defined.<br>
                ...
                <template #modal-footer="{ ok, cancel, hide }">
                    <base-button type="outline-primary" class="mr-auto"
                        @click="closeCompanyRemovalModal()">Cancel</base-button>
                    <base-button type="primary"
                        @click="move_model.move_company ? moveCompanyThenDelete() : deleteCompany()">
            ...
            <card class="no-border-card" body-classes="px-0 pb-1" footer-classes="pb-2">
                ...
                <template slot="header">
                    ...
                    <base-button @click.native="showCreateCompanyModal()" :disabled="!userCan('Add', 'Entities') || loading"
                        class="btn" size="sm" type="primary" v-b-tooltip.hover.top
                        title="An Entity may contain various resources (Such as users, roles, nodes, sensors, etc) and is the top-level container object in MAB."
                        icon>
                        Create Entity
                    </base-button>

                    <base-button @click.native="goToDevicePlacementReporting()"
                        :disabled="loading || (!isDistributor() && !isAdmin())" class="btn" size="sm" type="primary"
                        v-b-tooltip.hover.top title="Entity Related Reporting" icon>
                        Reporting
                    </base-button>
                ...
                <b-row class='nomargin'>
                    ...
                    <b-col md>
                        <base-input prepend-icon="fas fa-search">
                            <b-input v-model="filterText" :debounce="1000" @update="loadCompanies" placeholder="Search...">
                            </b-input>
                        </base-input>
                    </b-col>
                </b-row>
                ...
                <b-row class='nomargin'>
                    <b-col md>
                        <b-table striped bordered outlined small stacked="lg" responsive show-empty primary-key="id"
                            ...
                            <template #cell(parent_entity)="data" v-if="isAdmin() || isDistributor()">
                                ...
                                <b-icon v-else variant="success" icon="asterisk" title="Top Level Entity"
                                    aria-hidden="true"></b-icon>
                            </template>

                            <template #cell(is_distributor)="data" v-if="isAdmin()">
                            ...
                            <template #cell(actions)="data" v-if="canAction">
                                <div class="d-flex justify-content-center">
                                    <b-button :disabled="!userCan('Edit', 'Entities', data.item.id, 'C')"
                                        @click="handleEdit(data.index, data.item)" class="btn" size="sm"
                                        variant="outline-primary" icon>
                                        Configure
                                    </b-button>
                                    <b-button :disabled="!userCan('Delete', 'Entities', data.item.id, 'C')"
                                        @click="openCompanyRemovalModal(data.index, data.item)" class="btn" size="sm"
                                        variant="outline-primary" icon>
                                        Remove
                                    </b-button>
                                </div>
                ...
                <div slot="footer" class='align-right'>
                    <b-row>
                        <b-col md>
                            Showing {{ Math.min(1 + (perPage * (currentPage - 1)), totalRows) }} to {{ Math.min((perPage *
                                (currentPage
                                    - 1))
                                + perPage, totalRows) }} of {{ totalRows }} entries
                        </b-col>
                        <b-col md>
                            <b-pagination @input="loadCompanies" v-model="currentPage" :total-rows="totalRows"
    ...
    methods: {
        ...
        loadCompanies() {
            ...
            this.$axios.post("/api/companies", {
                ...
                // this.model.companies = resp.tableData.company_name;

                if (this.totalRows == 0) { this.currentPage = 1; }

                if (this.bInitialQuery) {
                    this.formSteps = ['enttype', 'basic', 'paddr', 'baddr', 'modules'];
                    if (this.userCan('Add', 'Entities')) { this.formSteps.push('entities'); }
                    // if (this.userCan('Add', 'Groups')) { this.formSteps.push('groups'); }
                    // if (this.isAdmin()) { this.formSteps.unshift('enttype'); }
                    // if (this.isAdmin()) { this.formSteps.push('groups'); }
                    // this.formSteps = ['basic', 'paddr', 'baddr', 'modules', 'enttype'];
        ...
        showCreateCompanyModal() {
            ...
            // let companies = this.tableData;

            // let comp = companies.forEach(function (company) {
            //   if (company.is_distributor == 0) {
...
<style>.vue-advanced-cropper__background,
