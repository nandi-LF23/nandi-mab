    ...
    <b-container fluid class="mt--6">
      ...
      <b-modal v-model="showNewNodeModal" centered no-close-on-esc no-close-on-backdrop>
          ...
          <validation-observer ref='addstep_node' slim>
            <form role="addstep_node" v-show="formStep == 'node'" autocomplete="off" @submit.prevent="() => false">
              ...
              <b-row>
                <b-col>
                  <!-- Node Address -->
                  <base-input vid="node_address" rules="required" name="node address" placeholder="Node Address"
                    v-model="model.node_address" autocomplete="off">
                  </base-input>
                </b-col>

                <b-col>
                  <!-- Probe Address -->
                  <base-input vid="probe_address" name="probe address" @blur="runPrechecks" placeholder="Probe Address"
                    v-model="model.probe_address">
                  </base-input>
                </b-col>
              </b-row>
              ...
              <b-row>
                <b-col>
                  ...
                  <base-input name="date" rules="required" vid="commissioning_date">
                    <flat-picker slot-scope="{focus, blur}" @on-open="focus" @on-close="blur"
                      placeholder="Commissioning Date" class="form-control datepicker" v-model="model.commissioning_date">
                    </flat-picker>
                  </base-input>
                </b-col>
                ...
                <b-col>
                  <!-- Field Zone ID -->
                  <base-input vid="zone" rules="integer" name="zone" placeholder="Field Zone ID"
                    v-model="model.zone"></base-input>
                </b-col>
              </b-row>
              ...
              <b-row>
                <b-col>
                  <!-- Field Name -->
                  <base-input vid="field_name" rules="required" name="field name" placeholder="Field Name"
                    v-model="model.field_name"></base-input>
                </b-col>
              </b-row>
              ...
              <b-row>
                <b-col>
                  <!-- Latitude -->
                  <base-input vid="latt" :rules="{ regex: /^-?\d+(\.\d{1,15})?$/ }" placeholder="Latitude" name="latitude"
                    v-model="model.latt"></base-input>
                </b-col>

                <b-col>
                  <!-- Longitude -->
                  <base-input vid="lng" :rules="{ regex: /^-?\d+(\.\d{1,15})?$/ }" placeholder="Longitude"
                    name="longitude" v-model="model.lng"></base-input>
                </b-col>

                <b-col>
                  <!-- Visual Coordinate Picker -->
                  <MapCoordsPicker :fullWidthBtn="true" :accessToken="$store.state.mapBoxAccessToken"
                    modalTitle="Pinpoint Coordinates" buttonTitle="Set Coords" :startPos="startPos"
                    @setCoords="setCoords">
                  </MapCoordsPicker>
                </b-col>
              </b-row>
                  ...
                  <base-input name="node type" rules="required" vid="node_type">
                    <el-select @change="nodeTypeChanged" v-model="model.node_type" filterable placeholder="Node Type">
                      ...
                      <el-option :disabled="!userCan('Add', 'Node Config') && !userCan('Add', 'Nutrients')"
                        label="Nutrient" value="Nutrients" v-if="isAdmin()"></el-option>
                      <el-option :disabled="!userCan('Add', 'Node Config') && !userCan('Add', 'Well Controls')"
                        label="Well Controls" value="Wells"></el-option>
                      <el-option :disabled="!userCan('Add', 'Node Config') && !userCan('Add', 'Meters')" label="Meters"
                        value="Water Meter"></el-option>
                    </el-select>
                  </base-input>
                </b-col>
                <b-col>
                  <!-- Weather Station Name -->
                  <base-input placeholder="W.Station Name" name="wl_station_name" vid="wl_station_name"
              ...
              <b-row>
                <b-col>
                  <!-- Entity -->
                  <base-input name="company" rules="required" vid="company" placeholder="Entity" autocomplete="off">
                    <el-select :disabled="!model.node_type" @change="companyChanged" v-model="model.company_id" filterable
                      placeholder="Entity" autocomplete="off">
                      <template v-if="model.node_type && Object.values(companies).length">
                        <template v-for="company in companies">
                          <el-option v-if="userCan('Add', convertNodeTypeToSubsystem(model.node_type), company.id, 'C') ||
                ...
                <b-col>
                  <!-- Group -->
                  <base-input name="group" vid="group" placeholder="Group" v-b-tooltip.hover.top
                    title="Optionally add this node to a security group." autocomplete="off">
                    <el-select :disabled="!model.node_type || !model.company_id" v-model="model.group_id" filterable
                      placeholder="Group" autocomplete="off">
                      <template v-if="model.node_type && groups.length">
                        <template v-for="group in groups">
                          <el-option v-if="userCan('Add', convertNodeTypeToSubsystem(model.node_type), group.id, 'G') ||
                            userCan('Add', 'Node Config', group.id, 'G')
                            " :key="group.id" :label="group.group_name" :value="group.id">
                ...
                <b-col md>
                  ...
                  <base-input name="node make" rules="required" vid="node_make">
                    <el-select v-model="model.node_make" default-first-option filterable placeholder="Node Make">
                      ...
                      <el-option label="DMT (Hawk)" value="DMT (Hawk)"></el-option>
                      <el-option label="Other" value="Other"></el-option>
                    </el-select>
                  </base-input>
                </b-col>
                <b-col md>
                  <base-input name="node serial no" rules="required" placeholder="Node Serial No" vid="node_serial_number"
                    v-model="model.node_serial_number"></base-input>
                </b-col>
              </b-row>
              ...
              <template v-if="model.node_type === 'Soil Moisture'">
                <b-row>
                  ...
                  <b-col>
                    ...
                    <base-input name="device serial no" rules="required" placeholder="Device Serial No"
                      vid="device_serial_number" v-model="model.device_serial_number"></base-input>
                  </b-col>
                </b-row>
              </template>
                  ...
                  <b-col>
                    <base-input name="nutrient template" rules="required" vid="nutrient_template">
                      <el-select v-model="model.nutrient_template_id" filterable placeholder="Nutrient Template">
                        <el-option v-for="tpl in nutrient_templates" :key="tpl.id" :label="tpl.name"
                ...
                <b-col>
                  <b-table-simple responsive small fixed>
                    ...
                    <b-tr v-if="model.hardware_management_id">
                      <b-td><strong>Device Make</strong></b-td><b-td>{{
                        device_types[model.hardware_management_id].device_make }}</b-td>
                    </b-tr>
                    <b-tr><b-td><strong>Node Serial No</strong></b-td><b-td>{{ model.node_serial_number }}</b-td></b-tr>
                    <b-tr><b-td><strong>Device Serial No</strong></b-td><b-td>{{ model.device_serial_number
                    }}</b-td></b-tr>
                    <b-tr><b-td><strong>Commissioning Date</strong></b-td><b-td>{{ model.commissioning_date
                    }}</b-td></b-tr>
        ...
        <template #modal-footer="{ ok, cancel, hide }">
          ...
          <base-button type="outline-primary" class="ml-auto" @click="prevStep()"
            v-show="formStep != 'node'">Previous</base-button>
          <base-button type="primary" @click="formStep == 'confirm' ? addNode() : nextStep()"> {{ formStep == 'confirm' ?
            'Create' : 'Next' }}</base-button>
        </template>

      </b-modal>

      <!-- Import -->
      <b-modal v-if="userCan('Import', 'Node Config')" v-model="showImportModal" centered no-close-on-esc
        ...
        <template #default="{ hide }">
          <validation-observer ref="importform" slim>
            <form role="form" autocomplete="nope" @submit.prevent="() => false">
                  ...
                  <base-input label="Data Format" name="data format" rules="required" vid="format">
                    <el-select v-model="import_model.format" filterable placeholder="Data Format">
                      <template v-for="(format, iidx) in importFormats">
                        <el-option v-if="format.admin == true ? isAdmin() : true" :label="format.label"
                          :value="format.value" :key="iidx">
                ...
                <b-col md>
                  <validation-provider v-slot="{ errors, validate }">
                    <base-input label="Data File" name="file" rules="required" vid="filedata">
                      <b-form-file v-model="import_model.filedata" @change="fileSelectionChange(validate)"
              ...
              <b-row v-if="import_model.format && import_model.format != 'node_import'">
                <b-col md="6">
                  <base-input label="CSV Delimiter" name="delimiter" rules="required" vid="delimiter">
                    <el-select v-model="import_model.delimiter" filterable placeholder="CSV Delimiter">
                      <el-option v-for="(item, didx) in delimiters" :key="didx" :label="item.label" :value="item.value">
                      </el-option>
                    </el-select>
                  </base-input>
                </b-col>
                <b-col md="6">
                  <base-input label="Date Timezone" name="timezone" rules="required" vid="timezone">
                    <el-select v-if="timeZones.length" v-model="import_model.timezone" filterable placeholder="Timezone">
                      <el-option v-for="item in timeZones" :key="item._key" :label="item._lab" :value="item._key">
                      </el-option>
                    </el-select>
                  </base-input>
                </b-col>
                <b-col md="6">
                  <transition name="fade">
                    <base-input v-if="import_model.format == 'partial_n'" label="Date Format" name="date format"
                      rules="required" vid="date_format">
                      <el-select v-if="dateFormats.length" v-model="import_model.date_format" filterable
                        placeholder="Date Format">
                        <el-option v-for="item in dateFormats" :key="item._key" :label="item._lab" :value="item._key">
                        </el-option>
                      </el-select>
                    </base-input>
                  </transition>
              ...
              <b-row>
                <b-col md>
                  <b-form-checkbox v-model="import_model.dryrun" :value="'true'" :unchecked-value="'false'">Dry
                    run</b-form-checkbox>
                </b-col>
              </b-row>
              ...
              <b-row v-if="import_model.log">
                <b-col md>
                  <b-form-textarea class="importlog" ref="importlog" v-model="import_model.log" placeholder="Log output.."
              ...
              <b-row v-if="import_model.progress">
                <b-col md>
                  <b-progress :max="100" height="2rem">
                    <b-progress-bar :value="import_model.progress" :label="import_model.progress + '%'"
                      :variant="import_model.variant" :animated="!import_model.complete">
                    </b-progress-bar>
                  </b-progress>
                </b-col>
              </b-row>
        ...
        <template #modal-footer="{ ok, cancel, hide }">
          <base-button type="outline-primary" class="ml-auto"
            @click="import_model.busy ? cancelImport() : closeImportModal()" :disabled="import_model.complete">
            Cancel
          </base-button>
          <base-button type="primary" @click="import_model.complete ? closeImportModal() : importFile()"
            :disabled="import_model.busy && !import_model.complete">
            {{ (import_model.busy && !import_model.complete) ? "Busy" : (import_model.complete ? "Done" : "Import") }}
          </base-button>
        </template>
      </b-modal>

      <b-modal v-if="userCan('Import', 'Node Config')" v-model="showFormatPreviewModal" size="xl" centered no-close-on-esc
        no-close-on-backdrop>
        <template #modal-header="{ close }">
        ...
        <template #default="{ hide }">
          <CodeEditor v-model="previewModalContent" :read_only="true" :wrap_code="true" :hide_header="true" theme="light"
            width="100%" border_radius="0px" height="200px" :languages="[['markdown', 'Markdown']]" font_size="14px">
          </CodeEditor>
        </template>
      ...
      <card class="no-border-card" body-classes="px-0 pb-0" footer-classes="pb-2">
        <template slot="header">
          ...
          <base-button :disabled="!userCan('Add', 'Node Config') &&
            !userCan('Add', 'Soil Moisture') &&
            !userCan('Add', 'Nutrients') &&
            !userCan('Add', 'Well Controls') &&
            !userCan('Add', 'Meters') ||
            loading" @click.native="showAddNodeModal()" class="btn" size="sm" type="primary" v-b-tooltip.hover.top
            title="Add a new hardware node to an existing entity." icon>
            Add Node
          </base-button>

          <base-button :disabled="!userCan('Import', 'Node Config') || loading" @click.native="openImportDataModal()"
            class="btn" type="primary" size="sm" v-b-tooltip.hover.top title="Import non-live data/metadata via CSV."
            icon>
            Import
          </base-button>
          ...
          <b-modal id="modal-4" title="Node Configuration">
            <p>Managing devices in MAB revolves around the concept of the <em>Node</em>.
            </p>
            <p>Each node is an IoT (Internet of Things) hardware device that's usually
              installed within a single Crop Field. A node is normally connected to one or more sensors of varying type.
            </p>
            <p>The node collects data (via it's sensors) from the ground soil and
              transmits it at regular intervals to MAB for processing and storage. The data is then later accessed,
              analyzed and interpreted by an agronomist and/or grower.</p>
            <p>In MAB, each node represents one sensor. Thus, for a node to have
              multiple sensors, multiple nodes need to be created and assigned a sensor configuration each. Every node has
              a type that corresponds to it's sensor's type.&nbsp;</p>
            <p>A node can be of varying type as well as manufacturer. MAB aims to
              normalize different hardware devices into a unified management interface.</p>
            <p>Node management is accessed via the <strong>Node Config</strong> section
              (visible and accessible via the side menu, if granted via the user's role).</p>
            <h2>The Node Config Screen</h2>
            <p>The Node Config screen display's all of the installed nodes that are
              available to the currently logged in user in a table layout.<br>Below is a summary of each table column:<br>
            </p>
            <div class="table-responsive">
              <table class="table table-sm">
                <tbody>
                  ...
                  <tr>
                    <td>Node Address</td>
                    <td>The Node's uniquely identifying address</td>
                    <td><br></td>
                  </tr>
                  <tr>
                    <td>Node Type</td>
                    <td>The Type of Node</td>
                    <td>All (Types determined by role)</td>
                  </tr>
                  <tr>
                    <td>Last Update</td>
                    <td>When MAB last received data</td>
                    <td>Formatted with the user's time zone</td>
                  </tr>
                  <tr>
                    <td>Field Name</td>
                    <td>The Field in which the Node was installed</td>
                    <td><br></td>
                  </tr>
                  <tr>
                    <td>Latitude</td>
                    <td>The Node's Latitudinal Coordinate</td>
                    <td>Automatic or Manually set</td>
                  </tr>
                  <tr>
                    <td>Longitude</td>
                    <td>The Node's Longitudinal Coordinate</td>
                    <td>Automatic or Manually set</td>
                  </tr>
                  <tr>
                    <td>Integrations</td>
                    <td>Any Third-Party integrations enabled</td>
                    <td>Visible to Admins and Manager roles</td>
                  </tr>
                  <tr>
                    <td>Actions</td>
            ...
            <p>To add a new node to your Entity, locate and click on the <strong>Add
                Node</strong> button at the top left corner of the Node Configuration screen. This will open the
              multi-step <strong>Add New Node</strong> dialog. Fill in all of the required fields for each step (Node
              Details/Configuration) and click on the <strong>Next</strong> button to proceed to each section.</p>
            <p>Once you reach the summary section, please confirm that all data was
              entered correctly and then click on the <strong>Create </strong>button. This will create the node's main
              configuration record and if successful, a green success notification (top right) would display.&nbsp;</p>
            <h2>Removing an Existing Node</h2>
            <p>To remove a previously added node, locate it in the Node Config table
              (either manually or via the Search Input located to the top right of the table). Then, click on the Record's
              <strong>Remove</strong> button (in the actions column). A confirmation dialog will appear. To confirm and
              remove the node, click on <strong>Remove</strong>.<br><br><strong>Please Note:</strong> Removing a node's
              configuration record does not remove the captured/collected data readings for that node. This is&nbsp; by
              design to allow recreating a node without incurring any data loss. The proper permissions are required to
              remove a node.
            </p>
            <h2>Rebooting a Node</h2>
            <p>Due to various hardware-related reasons, Node's can sometimes become
              unresponsive. Therefore, MAB supports rebooting a Node to try and get it up and running again.</p>
            <p>To reboot a Node, locate the Node's record in the Node Config table.
              Then, click on the Record's <strong>Reboot </strong>button (in the actions column). A confirmation dialog
              will appear. To confirm and reboot the Node, click on <strong>Reboot</strong>.</p>
            <p><strong>Please Note:</strong> If your account doesn't have permissions to
              reboot a node, the reboot button would be disabled and unclickable.</p>
            <h2>Configure an Existing Node</h2>
            <p>In order to manage a node's configuration, a dedicated node configuration
              screen is used. To access each Node's configuration screen, locate the <strong>Configure</strong> button on
              the Node Config table and click on it.</p>
          </b-modal>
        </template>
        ...
        <b-row class='nomargin'>
          ...
          <b-col md>
            <el-select v-if="companies && Object.values(companies).length > 1" class="fullwidth" clearable filterable
              placeholder="Filter by Entity.." v-model="filterEntity" @change="loadHardwareConfig">
              <el-option v-for="c in companies" :value="c.id" :label="c.company_name" :key="c.id"></el-option>
            </el-select>
          </b-col>
          <b-col md>
            <base-input prepend-icon="fas fa-search">
              <b-input v-model="filterText" :debounce="1000" @update="loadHardwareConfig" placeholder="Search...">
              </b-input>
            </base-input>
          </b-col>
        </b-row>
            ...
            <b-table striped bordered outlined small stacked="lg" responsive show-empty primary-key="node_address"
              ...
              <template #cell(actions)="data" v-if="canAction">
                <div class="d-flex justify-content-center">
                  <b-button :disabled="!userCan('Edit', 'Node Config', data.item.id, 'O') &&
                  ...
                  <b-button :disabled="!userCan('Reboot', 'Node Config', data.item.id, 'O')"
                    @click="handleReboot(data.index, data.item)" class="btn" variant="outline-primary"
                    title="Attempt to reboot this node." size="sm">
                    Reboot
                  </b-button>

                  <b-button :disabled="!userCan('Delete', 'Node Config', data.item.id, 'O') &&
                    !userCan('Delete', convertNodeTypeToSubsystem(data.item.node_type), data.item.id, 'O')"
                    @click="handleDelete(data.index, data.item)" class="btn" variant="outline-primary"
                    title="Remove this node from it's entity. Note: It's feed data would still remain in tact." size="sm">
                    Remove
                  </b-button>
                </div>
              </template>
        ...
        <div slot="footer" class='align-right'>
          <b-row>
            ...
            <b-col md>
              <b-pagination @input="loadHardwareConfig" v-model="currentPage" :total-rows="totalRows" :per-page="perPage"
...
export default {
  ...
  methods: {
    ...
    loadHardwareConfig(sortByOverride = '', sortDirOverride = '') {
      this.loading = true;
      this.$axios.post("/api/hardwareconfigtable", {
    ...
    loadNutrientTemplates() {
      if (this.model.company_id && this.model.node_type == 'Nutrients') {
        this.$axios.get("/api/loadNutrientTemplates/" + this.model.company_id)
          .then((response) => {
    ...
    addNode() {
      ...
      if (this.formsValid) {
        ...
        this.$axios.post("/api/hardwareconfignew", this.model)
          .then((resp) => {
            this.loading = false;
    ...
    deleteNode(row) {
      this.loading = true;
      this.$axios.post("/api/hardwareconfigdestroy", { node_address: row.node_address })
        .then((resp) => {
          this.loading = false;
    ...
    rebootNode(row) {
      this.loading = true;
      this.$axios.get("/api/hardwareconfigreboot/" + row.node_address)
        .then((resp) => {
          this.loading = false;
    ...
    runPrechecks(e) {
      if (this.model.node_address && this.model.probe_address != '') {
        ...
        this.$axios.get("/api/hardwareconfigexists/" + node_address)
          .then((resp) => {
    ...
    */
    ...
    tryAndGetCoords(node_address) {
      ...
      if (node_address) {
        this.$axios.get("/api/hardwareconfiglatestcoords/" + node_address)
          .then((resp) => {
