<template>
  <div class='map_container fieldmap'>
    <FocusMenu @focusMenuChanged="toggleBurger" ref='focusmenu'>
      ...
      <b-button style="margin-left: 8px !important" v-b-modal.modal-4 size="sm" variant="primary" class="btn"
        v-b-tooltip.hover.top title="Read More">
        <b-icon icon="question-circle-fill" aria-label="Help"></b-icon>
      </b-button>
      <b-modal id="modal-4" title="Field Management">
          <p>The Field Management screen is similar to the Map Screen but is more geared
            towards managing fields and field zones.<br>Field Management features include:</p>
          <h3>Visual Text Search</h3>
          <p>Simply type in the search query (Field Names / Node Addresses) and see the
            results filter through (as well as visually focus).</p>
          <h3>Filtering by Node Type</h3>
          <p>Clicking on a node type button will only display nodes (and fields) of that
            type and hide the others.</p>
          <h3>Detail display of each field</h3>
          <p>Clicking on a field perimeter will bring up a popup with various information
            related to the field.</p>
          <h3>Map Legends</h3>
          <p>A map legend dialog can be shown by clicking on the Question Mark Button on
            the lower right corner of the map.</p>
      </b-modal>
      <template v-for="(     marker, index     ) in      markers     ">
        <div :key="nodeFilterType + index" v-if="marker.visible" class='field_record' @click=" focusMarker(marker)">

          <div class='field_name'>{{ marker.field_name }}
            <b-icon @click=" showFieldModal(marker.node_address)" icon="info-circle" aria-label="More"></b-icon>
          </div>
    ...
    <b-modal class="field-visible" dialog-class='mab_modal' v-model="fieldModalVisible" centered hide-backdrop no-enforce-focus size='lg'>
...
.field-visible  .modal-body {
  padding: 10px;
}

.field-visible .modal-footer .btn {
  position: absolute;
  bottom: 15%;
}

.field-visible .modal-footer {
  padding: 0;
  padding-right: 10px;
}
